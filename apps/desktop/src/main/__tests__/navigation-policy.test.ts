...
void describe('classifyMainFrameNavigationUrl', () => {
	void it('allows the packaged renderer index file', () => {
		const result = classifyMainFrameNavigationUrl(pathToFileURL(packagedIndexPath).toString(), {
			packagedIndexPath,
		});
	...
	void it('allows packaged renderer index file hash navigations', () => {
		const result = classifyMainFrameNavigationUrl(`${pathToFileURL(packagedIndexPath).toString()}#settings`, {
			packagedIndexPath,
		});
	...
	void it('denies other packaged file urls without externalizing', () => {
		const result = classifyMainFrameNavigationUrl(
			pathToFileURL(path.resolve('/tmp/ripcord/resources/app.asar/renderer-dist/other.html')).toString(),
			{
				packagedIndexPath,
			},
		);
	...
	void it('allows same-origin dev renderer navigations', () => {
		const result = classifyMainFrameNavigationUrl('http://localhost:5173/debug?panel=voice', {
			packagedIndexPath,
	...
	void it('denies lookalike dev origins', () => {
		const result = classifyMainFrameNavigationUrl('http://localhost.evil.example:5173', {
			packagedIndexPath,
			rendererUrl: 'http://localhost:5173',
		});
	...
	void it('denies unsafe and malformed urls without externalizing', () => {
		const javascriptResult = classifyMainFrameNavigationUrl('javascript:alert(1)', {
			packagedIndexPath,
		});
