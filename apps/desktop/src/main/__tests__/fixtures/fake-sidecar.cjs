...
const capabilitySystemAudio = process.env.FAKE_SIDECAR_SYSTEM_AUDIO || 'supported';
const capabilityPerAppAudio = process.env.FAKE_SIDECAR_PER_APP_AUDIO || 'supported';
...
const capabilityLinuxAudioBackend = process.env.FAKE_SIDECAR_LINUX_AUDIO_BACKEND;
...
const capabilityLinuxAudioRuntimeAvailable = parseBooleanEnv(
	process.env.FAKE_SIDECAR_LINUX_AUDIO_RUNTIME_AVAILABLE ?? process.env.FAKE_SIDECAR_PIPEWIRE_RUNTIME_AVAILABLE,
);
const capabilityLinuxAudioRuntimeReason =
	process.env.FAKE_SIDECAR_LINUX_AUDIO_RUNTIME_REASON || process.env.FAKE_SIDECAR_PIPEWIRE_RUNTIME_REASON;
const capabilityLinuxAudioCaptureAvailable = parseBooleanEnv(
	process.env.FAKE_SIDECAR_LINUX_AUDIO_CAPTURE_AVAILABLE ?? process.env.FAKE_SIDECAR_PIPEWIRE_TOOLS_AVAILABLE,
);
const capabilityPortalAvailable = parseBooleanEnv(process.env.FAKE_SIDECAR_PORTAL_AVAILABLE);
...
const capabilityAppAudioTargetEnumerationReason = process.env.FAKE_SIDECAR_APP_AUDIO_TARGET_ENUMERATION_REASON;
const capabilityAppAudioTargetEnumerationReasonCode = process.env.FAKE_SIDECAR_APP_AUDIO_TARGET_ENUMERATION_REASON_CODE;
...
const capabilitySourceAudioTargetInferenceReason = process.env.FAKE_SIDECAR_SOURCE_AUDIO_TARGET_INFERENCE_REASON;
const capabilitySourceAudioTargetInferenceReasonCode =
	process.env.FAKE_SIDECAR_SOURCE_AUDIO_TARGET_INFERENCE_REASON_CODE;
const capabilityGlobalPushKeybinds = process.env.FAKE_SIDECAR_GLOBAL_PUSH_KEYBINDS;
const capabilityGlobalPushKeybindsReason = process.env.FAKE_SIDECAR_GLOBAL_PUSH_KEYBINDS_REASON;
const capabilityGlobalPushKeybindsReasonCode = process.env.FAKE_SIDECAR_GLOBAL_PUSH_KEYBINDS_REASON_CODE;
const capabilityX11DisplayAvailable = parseBooleanEnv(process.env.FAKE_SIDECAR_X11_DISPLAY_AVAILABLE);
const capabilityX11DisplayReason = process.env.FAKE_SIDECAR_X11_DISPLAY_REASON;
const capabilityX11DisplayReasonCode = process.env.FAKE_SIDECAR_X11_DISPLAY_REASON_CODE;
...
const capabilityLinuxGlobalShortcutsPortalBackend = process.env.FAKE_SIDECAR_LINUX_GLOBAL_SHORTCUTS_PORTAL_BACKEND;
const capabilityLinuxGlobalShortcutsPortalReason = process.env.FAKE_SIDECAR_LINUX_GLOBAL_SHORTCUTS_PORTAL_REASON;
const listTargetsRequiresManualSelection = parseBooleanEnv(process.env.FAKE_SIDECAR_REQUIRES_MANUAL_SELECTION);
...
const pushTalkRegisteredOverride = parseBooleanEnv(process.env.FAKE_SIDECAR_PUSH_TALK_REGISTERED);
const pushMuteRegisteredOverride = parseBooleanEnv(process.env.FAKE_SIDECAR_PUSH_MUTE_REGISTERED);
const pushKeybindErrors = parseStringArrayEnv(process.env.FAKE_SIDECAR_PUSH_KEYBIND_ERRORS) || [];
const emitTalkActiveOnSet = parseBooleanEnv(process.env.FAKE_SIDECAR_EMIT_PUSH_TALK_ACTIVE_ON_SET);
const emitMuteActiveOnSet = parseBooleanEnv(process.env.FAKE_SIDECAR_EMIT_PUSH_MUTE_ACTIVE_ON_SET);
...
rl.on('line', (line) => {
	...
	if (method === 'audio_targets.list') {
		...
		sendResponse(id, {
			...
			requiresManualSelection:
				listTargetsRequiresManualSelection ?? (capabilityPlatform === 'linux' || suggestedTargetId === undefined),
			warning:
				listTargetsWarning ??
				(capabilityPlatform === 'linux'
	...
	if (method === 'push_keybinds.set') {
		const talkRegistered = pushTalkRegisteredOverride ?? Boolean(params?.pushToTalkKeybind);
		const muteRegistered = pushMuteRegisteredOverride ?? Boolean(params?.pushToMuteKeybind);
	...
	if (method === 'capabilities.get') {
		sendResponse(id, {
			...
			appAudioTargetEnumerationSupported: capabilityAppAudioTargetEnumerationSupported,
			appAudioTargetEnumerationReason: capabilityAppAudioTargetEnumerationReason,
			appAudioTargetEnumerationReasonCode: capabilityAppAudioTargetEnumerationReasonCode,
			sourceAudioTargetInferenceSupported: capabilitySourceAudioTargetInferenceSupported,
			sourceAudioTargetInferenceReason: capabilitySourceAudioTargetInferenceReason,
			sourceAudioTargetInferenceReasonCode: capabilitySourceAudioTargetInferenceReasonCode,
			...
			linuxGlobalShortcutsPortalConfigured: capabilityLinuxGlobalShortcutsPortalConfigured,
			linuxGlobalShortcutsPortalBackend: capabilityLinuxGlobalShortcutsPortalBackend,
			linuxGlobalShortcutsPortalReason: capabilityLinuxGlobalShortcutsPortalReason,
		});
		return;
	}
