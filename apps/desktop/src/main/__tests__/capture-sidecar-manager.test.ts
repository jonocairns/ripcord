...
import { CaptureSidecarManager, toPcmAppAudioFrame } from '../capture-sidecar-manager';
import { getDesktopCapabilitiesForPlatform, resolvePreparedScreenAudioMode } from '../platform-capabilities';
import type { TAppAudioFrame, TAppAudioPcmFrame, TAppAudioStatusEvent, TDesktopPushKeybindEvent } from '../types';

const fakeSidecarPath = path.resolve(import.meta.dirname, 'fixtures', 'fake-sidecar.cjs');

const waitFor = async (predicate: () => boolean, timeoutMs = 2_000, intervalMs = 20) => {
...
void describe('CaptureSidecarManager', () => {
	...
	void it('starts capture and forwards frame/status events', async () => {
		...
		try {
			...
			await waitFor(() => statusEvents.some((event) => event.reason === 'capture_stopped'));
		} finally {
			offFrame();
			offPcmFrame();
			offStatus();
	...
	void it('validates sidecar capabilities and target list responses', async () => {
			...
			spawnSidecar: () => {
				return spawn(process.execPath, [fakeSidecarPath], {
					...
					env: {
						...
						FAKE_SIDECAR_SOURCE_AUDIO_TARGET_INFERENCE_REASON: 'Choose a target manually.',
						FAKE_SIDECAR_SOURCE_AUDIO_TARGET_INFERENCE_REASON_CODE: 'linux-manual-app-target-selection-required',
		...
		try {
			...
			assert.equal(capabilities.sourceAudioTargetInferenceReasonCode, 'linux-manual-app-target-selection-required');
			...
			assert.match(targets.warning ?? '', /choosing the app that is producing sound/i);
		} finally {
			await manager.dispose();
		}
	});
	...
	void it('combines Linux capability probing, target listing, and fallback capture start in one flow', async () => {
		...
		try {
			...
			const resolvedCapabilities = resolveDesktopCaptureCapabilities({
				...
				sidecarPerAppAudioSupported: sidecarCapabilities.perAppAudio !== 'unsupported',
				sidecarCapabilities,
			});
			...
			assert.match(prepared.warning ?? '', /Linux requires choosing a running app audio target/i);
			...
			spawnSidecar: () => {
				return spawn(process.execPath, [fakeSidecarPath], {
					...
					env: {
						...
						FAKE_SIDECAR_REASON: 'ScreenCaptureKit audio capture requires macOS 13 or newer.',
	...
	void it('returns push keybind registration failures from the sidecar', async () => {
			...
			spawnSidecar: () => {
				return spawn(process.execPath, [fakeSidecarPath], {
					...
					env: {
						...
						FAKE_SIDECAR_PUSH_KEYBIND_ERRORS: '["Push-to-talk requires an X11 display server connection."]',
					},
				});
			},
			restartDelayMs: 10,
		...
		try {
			...
			assert.deepEqual(result.errors, ['Push-to-talk requires an X11 display server connection.']);
		} finally {
			await manager.dispose();
		}
	});
	...
	void it('refreshes reported capabilities after the sidecar restarts with a different environment', async () => {
			...
			spawnSidecar: () => {
				...
				return spawn(process.execPath, [fakeSidecarPath], {
					...
					env: {
						...
						FAKE_SIDECAR_LINUX_AUDIO_BACKEND_USES_SHELL_OUTS: 'false',
						FAKE_SIDECAR_LINUX_AUDIO_RUNTIME_AVAILABLE: 'true',
						FAKE_SIDECAR_LINUX_AUDIO_CAPTURE_AVAILABLE: isFirstSpawn ? 'false' : 'true',
						FAKE_SIDECAR_PER_APP_AUDIO: isFirstSpawn ? 'unsupported' : 'best-effort',
						FAKE_SIDECAR_APP_AUDIO_TARGET_ENUMERATION_SUPPORTED: isFirstSpawn ? 'false' : 'true',
						...
						FAKE_SIDECAR_APP_AUDIO_TARGET_ENUMERATION_REASON_CODE: isFirstSpawn
							...
							: '',
						FAKE_SIDECAR_SOURCE_AUDIO_TARGET_INFERENCE_SUPPORTED: isFirstSpawn ? 'false' : 'true',
					},
				});
			},
			restartDelayMs: 20,
		...
		try {
			...
			assert.equal(refreshedCapabilities.appAudioTargetEnumerationSupported, true);
		} finally {
			await manager.dispose();
		}
	});
	...
	void it('restores push keybind registration after the sidecar exits and restarts', async () => {
		...
		try {
			...
			await waitFor(() => pushEvents.some((event) => event.kind === 'talk' && event.active));
			await waitFor(() => pushEvents.some((event) => event.kind === 'talk' && !event.active), 3_000);
			await waitFor(() => spawnCount >= 2, 3_000);
			await waitFor(() => pushEvents.filter((event) => event.kind === 'talk' && event.active).length >= 2, 3_000);
			...
			assert.equal(pushEvents.filter((event) => event.kind === 'talk' && event.active).length >= 2, true);
		} finally {
			offPushKeybind();
			await manager.dispose();
		}
	...
	void it('emits sidecar_exited status and recovers after restart', async () => {
		...
		try {
			...
			await waitFor(() => statusEvents.some((event) => event.reason === 'sidecar_exited'));
			...
			assert.equal(lifecycleEvents.filter((event) => event.kind === 'ready').length >= 2, true);
		} finally {
			offStatus();
			offLifecycle();
			await manager.dispose();
