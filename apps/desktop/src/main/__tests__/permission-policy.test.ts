...
void describe("isPermissionAllowed", () => {
  void it("allows core media permissions for a trusted requester", () => {
    for (const permission of [
      ...
      assert.equal(
        isPermissionAllowed(permission, { isTrustedRequester: true }),
        true,
        permission,
      );
    }
  ...
  void it("denies core media permissions for an untrusted requester", () => {
    for (const permission of [
      ...
      assert.equal(
        isPermissionAllowed(permission, { isTrustedRequester: false }),
        false,
        permission,
      );
    }
  ...
  void it("allows fullscreen regardless of requester (e.g. embedded video players)", () => {
    assert.equal(
      isPermissionAllowed("fullscreen", { isTrustedRequester: true }),
      true,
    );
    assert.equal(
      isPermissionAllowed("fullscreen", { isTrustedRequester: false }),
      true,
    );
  });
