...
void describe("isTrustedRendererUrl", () => {
  ...
  void it("trusts packaged renderer index hash and query urls", () => {
    ...
    assert.equal(
      isTrustedRendererUrl(`${base}#settings`, { packagedIndexPath }),
      true,
    );
    assert.equal(
      isTrustedRendererUrl(`${base}?panel=voice`, { packagedIndexPath }),
      true,
    );
  });
  ...
  void it("rejects other packaged file urls", () => {
    const otherUrl = pathToFileURL(
      path.resolve("/tmp/ripcord/resources/app.asar/renderer-dist/other.html"),
    ).toString();
  ...
  void it("rejects remote origins in packaged builds", () => {
    ...
    assert.equal(
      isTrustedRendererUrl("http://localhost:5173", { packagedIndexPath }),
      false,
    );
  });
  ...
  void it("rejects unsafe and malformed urls", () => {
    assert.equal(
      isTrustedRendererUrl("javascript:alert(1)", { packagedIndexPath }),
      false,
    );
    assert.equal(
      isTrustedRendererUrl("not a url", { packagedIndexPath }),
      false,
    );
  });
});
