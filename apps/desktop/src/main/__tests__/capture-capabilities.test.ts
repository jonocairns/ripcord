...
void describe('resolveDesktopCaptureCapabilities', () => {
	...
	void it('downgrades linux system and per-app audio when the native backend is unavailable', () => {
		...
		const resolved = resolveDesktopCaptureCapabilities({
			...
			sidecarCapabilities: {
				...
				appAudioTargetEnumerationReason: 'Failed to connect to the Linux audio server.',
				appAudioTargetEnumerationReasonCode: 'linux-native-audio-backend-unavailable',
		...
		assert.equal(resolved.perAppAudio, 'unsupported');
		assert.equal(resolved.sidecarAvailable, true);
		assert.equal(resolved.issues[0]?.code, 'linux-native-audio-backend-unavailable');
	...
	void it('keeps linux per-app audio available when the sidecar path is ready', () => {
		...
		assert.equal(resolved.globalPushKeybinds, 'best-effort');
		assert.equal(
			resolved.issues.find((issue) => issue.code === 'linux-manual-app-target-selection-required')?.title,
	...
	void it('downgrades linux audio when capture startup is unavailable despite successful target probing', () => {
		...
		assert.equal(resolved.perAppAudio, 'unsupported');
		assert.equal(
			resolved.issues.find((issue) => issue.code === 'linux-native-audio-backend-unavailable')?.message,
	...
	void it('surfaces linux global push keybind requirements as structured issues', () => {
		...
		const resolved = resolveDesktopCaptureCapabilities({
			...
			sidecarCapabilities: {
				...
				globalPushKeybindsReason: 'Global push keybinds require an X11 display server connection.',
				globalPushKeybindsReasonCode: 'linux-x11-display-required',
				x11DisplayAvailable: false,
				x11DisplayReason: 'Global push keybinds require an X11 display server connection.',
		...
		assert.equal(
			resolved.issues.find((issue) => issue.code === 'linux-x11-display-required')?.title,
	...
	void it('treats a configured Wayland shortcut portal as real support', () => {
		...
		const resolved = resolveDesktopCaptureCapabilities({
			...
			sidecarCapabilities: {
				...
				x11DisplayAvailable: false,
				x11DisplayReason: 'No X11 display was detected for the current Linux session.',
				x11DisplayReasonCode: 'linux-x11-display-required',
		...
		assert.equal(
			resolved.issues.some((issue) => issue.code === 'linux-wayland-global-shortcuts-portal-available'),
			false,
		);
		assert.match(resolved.notes.join(' '), /Global Shortcuts portal backend: gnome/i);
	});
	...
	void it('surfaces a Linux desktop portal issue for Wayland screen sharing', () => {
		...
		assert.equal(
			resolved.issues.find((issue) => issue.code === 'linux-desktop-portal-required')?.title,
			'Desktop portal unavailable',
		);
		assert.equal(
			resolved.issues.find((issue) => issue.code === 'linux-desktop-portal-required')?.affects.includes('screen-share'),
			true,
		);
	});
	...
	void it('keeps legacy Linux audio tooling failures mapped for older sidecar contracts', () => {
		...
		assert.equal(
			resolved.issues.find((issue) => issue.code === 'linux-pipewire-tools-missing')?.title,
	...
	void it('deduplicates linux backend issues when multiple reasons map to the same code', () => {
		...
		const resolved = resolveDesktopCaptureCapabilities({
			...
			sidecarCapabilities: {
				...
				sessionType: 'x11',
				linuxAudioCaptureAvailable: false,
				perAppAudioReason: 'Per-app capture stays disabled until the native Linux audio backend is ready.',
				perAppAudioReasonCode: 'linux-native-audio-backend-unavailable',
				appAudioTargetEnumerationSupported: false,
		...
		assert.equal(resolved.issues.filter((issue) => issue.code === 'linux-native-audio-backend-unavailable').length, 1);
	});
	...
	void it('surfaces macOS Screen Recording permission failures as a distinct issue', () => {
		...
		assert.equal(resolved.systemAudio, 'unsupported');
		assert.equal(resolved.perAppAudio, 'unsupported');
		assert.equal(resolved.issues[0]?.code, 'macos-screen-recording-permission-required');
		assert.equal(resolved.issues[0]?.title, 'Screen Recording permission required');
	});
	...
	void it('surfaces macOS version support failures as a distinct issue', () => {
		...
		const resolved = resolveDesktopCaptureCapabilities({
			baseCapabilities,
			sidecarAvailable: false,
			sidecarReason: 'ScreenCaptureKit audio capture requires macOS 13 or newer.',
			sidecarCapabilities: {
		...
		assert.equal(resolved.issues[0]?.title, 'macOS version unsupported for screen audio');
	});
});
