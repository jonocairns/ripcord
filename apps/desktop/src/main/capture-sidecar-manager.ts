...
const SIDECAR_BINARY_NAME = process.platform === 'win32' ? 'sharkord-capture-sidecar.exe' : 'sharkord-capture-sidecar';
...
const runtimeRequire: NodeJS.Require | undefined = typeof require === 'function' ? require : undefined;

type TElectronAppLike = {
	isPackaged?: boolean;
};

const SUPPORT_LEVELS: TSupportLevel[] = ['supported', 'best-effort', 'unsupported'];
...
const hasOptionalPositiveInteger = (value: Record<string, unknown>, key: string) => {
	return !(key in value) || value[key] === undefined || isPositiveInteger(value[key]);
};
...
const isPushKeybindKind = (value: unknown): value is TDesktopPushKeybindEvent['kind'] => {
...
const isElectronAppLike = (value: unknown): value is TElectronAppLike => {
	...
	return !('isPackaged' in value) || value.isPackaged === undefined || typeof value.isPackaged === 'boolean';
};

const isSidecarCapabilities = (value: unknown): value is TSidecarCapabilities => {
	if (!isRecord(value)) {
		return false;
	}

	if ('platform' in value && value.platform !== undefined && typeof value.platform !== 'string') {
		return false;
	}

	if ('systemAudio' in value && value.systemAudio !== undefined && !isSupportLevel(value.systemAudio)) {
		return false;
	}

	if ('perAppAudio' in value && value.perAppAudio !== undefined && !isSupportLevel(value.perAppAudio)) {
		return false;
	}
...
const isDesktopAppAudioTarget = (value: unknown): value is TDesktopAppAudioTargetsResult['targets'][number] => {
	if (!isRecord(value)) {
		return false;
	}
...
const isDesktopAppAudioTargetsResult = (value: unknown): value is TDesktopAppAudioTargetsResult => {
...
const isAppAudioSession = (value: unknown): value is TAppAudioSession => {
	...
	return (
		...
		(!('encoding' in value) || value.encoding === undefined || value.encoding === 'f32le_base64')
	);
};

const isGlobalPushKeybindRegistrationResult = (value: unknown): value is TGlobalPushKeybindRegistrationResult => {
	if (!isRecord(value)) {
		return false;
	}
...
const isAppAudioStatusEvent = (value: unknown): value is TAppAudioStatusEvent => {
	if (!isRecord(value)) {
		return false;
	}
...
const isDesktopPushKeybindEvent = (value: unknown): value is TDesktopPushKeybindEvent => {
	if (!isRecord(value)) {
		return false;
	}
...
const isAppAudioBinaryEgressInfo = (value: unknown): value is TAppAudioBinaryEgressInfo => {
	if (!isRecord(value)) {
		return false;
	}
...
const expectSidecarResult = <T>(response: unknown, guard: (value: unknown) => value is T, method: string): T => {
	if (!guard(response)) {
...
const isSidecarResponse = (value: unknown): value is TSidecarResponse => {
	...
	if ('error' in value && value.error !== undefined) {
		...
		if ('message' in value.error && value.error.message !== undefined && typeof value.error.message !== 'string') {
			return false;
		}
	}
...
const toPcmAppAudioFrame = (frame: TAppAudioFrame): TAppAudioPcmFrame | undefined => {
	...
	const expectedBytes = frame.frameCount * frame.channels * Float32Array.BYTES_PER_ELEMENT;
...
class CaptureSidecarManager {
	...
	private readonly spawnSidecarOverride: (() => ChildProcessWithoutNullStreams) | undefined;
	...
	async getStatus(): Promise<TSidecarStatus> {
		try {
			...
			if (
				...
				(capabilities.systemAudio !== 'supported' || capabilities.perAppAudio !== 'supported')
			) {
				return {
					available: false,
					reason: capabilities.reason || 'macOS screen audio capture is unavailable.',
				};
			}
			...
			const reason = error instanceof Error ? error.message : 'Unknown sidecar error';
	...
	async getCapabilities(): Promise<TSidecarCapabilities> {
		...
		return expectSidecarResult(response, isSidecarCapabilities, 'capabilities.get');
	}

	async listAppAudioTargets(sourceId?: string): Promise<TDesktopAppAudioTargetsResult> {
		const response = await this.sendRequest('audio_targets.list', {
			sourceId,
		});

		return expectSidecarResult(response, isDesktopAppAudioTargetsResult, 'audio_targets.list');
	}
	...
	async startAppAudioCapture(input: TStartAppAudioCaptureInput): Promise<TAppAudioSession> {
		if (!this.appAudioBinaryEgressUnsupported && Date.now() >= this.nextAppAudioBinaryEgressRetryAt) {
			void this.ensureAppAudioBinaryEgress().catch((error) => {
				...
				this.nextAppAudioBinaryEgressRetryAt = Date.now() + BINARY_APP_AUDIO_EGRESS_RETRY_DELAY_MS;
		...
		const session = expectSidecarResult(response, isAppAudioSession, 'audio_capture.start');
	...
	async setPushKeybinds(input: TDesktopPushKeybindsInput): Promise<TGlobalPushKeybindRegistrationResult> {
		this.lastRequestedPushKeybinds = input.pushToTalkKeybind || input.pushToMuteKeybind ? { ...input } : undefined;

		const response = await this.sendRequest('push_keybinds.set', input);

		return expectSidecarResult(response, isGlobalPushKeybindRegistrationResult, 'push_keybinds.set');
	}
	...
	private getCandidatePaths() {
		...
		if (isPackaged) {
			candidates.push(path.join(process.resourcesPath, 'sidecar', 'bin', process.platform, SIDECAR_BINARY_NAME));
			candidates.push(path.join(process.resourcesPath, 'sidecar', 'bin', SIDECAR_BINARY_NAME));

			return candidates;
		}

		candidates.push(path.resolve(__dirname, '..', '..', 'sidecar', 'bin', process.platform, SIDECAR_BINARY_NAME));
		candidates.push(path.resolve(__dirname, '..', '..', 'sidecar', 'target', 'release', SIDECAR_BINARY_NAME));
		candidates.push(path.resolve(process.cwd(), 'sidecar', 'bin', process.platform, SIDECAR_BINARY_NAME));
	...
	private startSidecarProcess() {
		...
		if (!sidecarBinaryPath) {
			throw new Error('Rust capture sidecar binary not found. Run `bun run build:sidecar` in apps/desktop.');
		}
	...
	private async restorePushKeybindsAfterRestart() {
		...
		try {
			const response = await this.sendRequestInternal('push_keybinds.set', this.lastRequestedPushKeybinds);
			const result = expectSidecarResult(response, isGlobalPushKeybindRegistrationResult, 'push_keybinds.set');

			if (!result.talkRegistered || !result.muteRegistered) {
				console.warn('[desktop] Push keybind restore after restart was only partially successful', result);
			}
		} catch (error) {
			console.warn('[desktop] Failed to restore global push keybinds after sidecar restart', error);
		}
	}
	...
	private handleSidecarLine(rawLine: string) {
		...
		if (isSidecarResponse(parsedLine)) {
			...
			if (parsedLine.ok) {
				...
				const message = parsedLine.error?.message || 'Unknown sidecar request failure';
		...
		if (isSidecarEvent(parsedLine)) {
			if (parsedLine.event === 'audio_capture.frame') {
				if (!isAppAudioFrame(parsedLine.params)) {
					console.warn('[desktop] Dropping malformed sidecar audio frame event');
					return;
				}
			...
			if (parsedLine.event === 'audio_capture.ended') {
				if (!isAppAudioStatusEvent(parsedLine.params)) {
					console.warn('[desktop] Dropping malformed sidecar audio status event');
					return;
				}
			...
			if (parsedLine.event === 'push_keybind.state') {
				if (!isDesktopPushKeybindEvent(parsedLine.params)) {
					console.warn('[desktop] Dropping malformed sidecar push-keybind event');
					return;
				}
	...
	private parseAppAudioBinaryFramePayload(payload: Buffer): TAppAudioPcmFrame | undefined {
		let offset = 0;
		...
		const pcmArrayBuffer = pcmSlice.buffer.slice(pcmSlice.byteOffset, pcmSlice.byteOffset + pcmSlice.byteLength);
	...
	private isAppAudioBinaryEgressUnsupportedError(error: unknown): boolean {
		...
		return /unknown method:\s*audio_capture\.binary_egress_info/i.test(error.message);
	}
	...
	private handleAppAudioBinaryEgressData(chunk: Buffer) {
		...
		while (buffer.length >= offset + 4) {
			...
			if (payloadLength <= 0 || payloadLength > MAX_BINARY_APP_AUDIO_FRAME_SIZE_BYTES) {
				console.warn('[desktop] Dropping invalid binary app-audio frame length', payloadLength);
				this.closeAppAudioBinaryEgressSocket();
				return;
			}
	...
	private async ensureAppAudioBinaryEgress(): Promise<void> {
		...
		this.appAudioBinaryEgressConnectPromise = (async () => {
			...
			const info = expectSidecarResult(response, isAppAudioBinaryEgressInfo, 'audio_capture.binary_egress_info');
