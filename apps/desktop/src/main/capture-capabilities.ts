...
import type { TDesktopCapabilities, TDesktopCapabilityIssue, TSupportLevel } from './types';
...
const appendIssue = (issues: TDesktopCapabilityIssue[], issue: TDesktopCapabilityIssue | undefined) => {
	if (!issue) {
		return;
	}
...
const createIssueFromCode = (code: string | undefined, message: string): TDesktopCapabilityIssue => {
	switch (code) {
		...
		case 'linux-manual-app-target-selection-required':
			return {
				...
				message,
				guidance: ['Choose the running application that is producing sound before starting screen share.'],
			};
		case 'linux-x11-display-required':
			return {
				...
				message,
				guidance: ['Use an X11 or XWayland session for global push-to-talk and push-to-mute.'],
			};
		case 'linux-xwayland-best-effort':
			return {
				...
				message,
				guidance: ['Wayland global keybinds currently depend on XWayland support in the active compositor.'],
			};
...
const resolveGlobalPushKeybinds = (
	...
	return sidecarCapabilities?.globalPushKeybinds ?? baseCapabilities.globalPushKeybinds;
};
...
const resolveDesktopCaptureCapabilities = ({
	...
	const linuxAudioRuntimeAvailable =
		sidecarCapabilities?.linuxAudioRuntimeAvailable ?? sidecarCapabilities?.pipewireRuntimeAvailable;
	const linuxAudioRuntimeReason =
		sidecarCapabilities?.linuxAudioRuntimeReason ?? sidecarCapabilities?.pipewireRuntimeReason;
	const linuxAudioCaptureAvailable = sidecarCapabilities?.linuxAudioCaptureAvailable;
	const resolvedSystemAudio =
		baseCapabilities.platform === 'linux' && linuxAudioCaptureAvailable === false
			? 'unsupported'
			: (sidecarCapabilities?.systemAudio ?? baseCapabilities.systemAudio);
	const globalPushKeybinds = resolveGlobalPushKeybinds(baseCapabilities, sidecarCapabilities);

	if (baseCapabilities.platform === 'linux') {
		const legacyLinuxAudioToolsUnavailable = sidecarCapabilities?.pipewireToolsAvailable === false;

		if (sidecarCapabilities?.sessionType) {
			appendNote(notes, `Linux session type: ${formatSessionTypeLabel(sidecarCapabilities.sessionType)}.`);
		}
		...
		appendIssue(
			issues,
			sidecarCapabilities?.portalAvailable === false && sidecarCapabilities.sessionType === 'wayland'
				? createIssueFromCode(
						sidecarCapabilities.portalReasonCode ?? 'linux-desktop-portal-required',
						sidecarCapabilities.portalReason ?? 'Wayland screen sharing requires an available desktop portal service.',
					)
				: undefined,
		);
		...
		appendIssue(
			...
			sidecarCapabilities?.sourceAudioTargetInferenceSupported === false
				? createIssueFromCode(
						sidecarCapabilities.sourceAudioTargetInferenceReasonCode ?? 'linux-manual-app-target-selection-required',
		...
		if (!sidecarAvailable || !sidecarPerAppAudioSupported || linuxAudioCaptureAvailable === false) {
	...
	if (!sidecarAvailable || !sidecarPerAppAudioSupported) {
		...
		appendIssue(
			...
			createIssueFromCode(
				sidecarCapabilities?.reasonCode ??
					(baseCapabilities.platform === 'macos' ? 'macos-screen-audio-unavailable' : 'desktop-sidecar-unavailable'),
				unavailableMessage,
			),
		);
		...
		return {
			...baseCapabilities,
			systemAudio: baseCapabilities.platform === 'macos' ? 'unsupported' : baseCapabilities.systemAudio,
			perAppAudio: 'unsupported',
			globalPushKeybinds,
