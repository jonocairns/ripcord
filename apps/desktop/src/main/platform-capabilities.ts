import type { TDesktopCapabilities, TResolvedScreenAudioMode, TScreenAudioMode, TScreenShareSelection } from './types';

const getDesktopCapabilitiesForPlatform = (platform: NodeJS.Platform): TDesktopCapabilities => {
	switch (platform) {
		case 'win32':
			return {
				platform: 'windows',
	...
	if (requestedMode === 'system') {
		if (capabilities.systemAudio === 'unsupported') {
			return {
				requestedMode,
				effectiveMode: 'none',
				warning: 'System audio is not supported on this platform. Continuing without shared audio.',
			};
		}
		...
		if (capabilities.systemAudio === 'best-effort') {
			return {
				...
				warning: 'System audio capture is best-effort on this platform and may fail.',
			};
		}
	...
	if (requestedMode === 'app') {
		if (capabilities.perAppAudio === 'unsupported') {
			if (capabilities.systemAudio !== 'unsupported') {
				return {
					requestedMode,
					effectiveMode: 'system',
					warning: 'Per-app audio is not supported. Falling back to system audio.',
				};
			}
			...
			return {
				requestedMode,
				effectiveMode: 'none',
				warning: 'Per-app audio is not supported on this platform. Continuing without shared audio.',
			};
		}
		...
		if (capabilities.perAppAudio === 'best-effort') {
			return {
				requestedMode,
				effectiveMode: 'app',
				warning: 'Per-app audio capture is best-effort on this platform and may fail.',
			};
		}
...
const resolvePreparedScreenAudioMode = (
	...
	if (selection.audioMode === 'app' && selection.sourceId.startsWith('screen:')) {
		const fallbackMode = capabilities.systemAudio === 'unsupported' ? 'none' : 'system';
	...
	const requiresExplicitAppTarget =
		resolved.effectiveMode === 'app' && !selection.appAudioTargetId && capabilities.platform === 'linux';
	...
	const fallbackMode = capabilities.systemAudio === 'unsupported' ? 'none' : 'system';
	const warningPrefix = 'Per-app audio on Linux requires choosing a running app audio target.';
