...
const readPreviewRuntimeConfig = (): TPreviewRuntimeConfig | null => {
	...
	const configPath = path.join(process.resourcesPath, PREVIEW_RUNTIME_CONFIG_FILENAME);
...
const applyPreviewRuntimeConfig = (config: TPreviewRuntimeConfig): TPreviewRuntimeConfig => {
	const userDataSuffix = config.userDataSuffix || DEFAULT_PREVIEW_USER_DATA_SUFFIX;
	const baseAppName = app.getName().trim() || 'Ripcord';
	const previewUserDataPath = path.join(app.getPath('appData'), `${baseAppName} ${userDataSuffix}`);
