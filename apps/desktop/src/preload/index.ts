...
const appAudioFrameSubscribers = new Set<(frame: TAppAudioFrame | TAppAudioPcmFrame) => void>();
...
const scheduleAppAudioFrameChannelReconnect = () => {
	...
	const delayMs = Math.min(
		...
		APP_AUDIO_CHANNEL_RECONNECT_BASE_DELAY_MS * 2 ** Math.max(0, appAudioFrameReconnectAttempts),
	);
...
const bindAppAudioFrameFallbackListener = () => {
	...
	ipcRenderer.on('desktop:app-audio-frame', (_event: unknown, frame: TAppAudioFrame) => {
	...
	appAudioFramePortPromise = new Promise<boolean>((resolve) => {
		...
		const onPortReady = (event: unknown) => {
			...
			appAudioFramePort.onmessage = (portEvent) => {
				...
				const byteOffset = typeof pcmByteOffset === 'number' && Number.isInteger(pcmByteOffset) ? pcmByteOffset : 0;
				const byteLength =
					typeof pcmByteLength === 'number' && Number.isInteger(pcmByteLength) ? pcmByteLength : pcmBuffer.byteLength;
				...
				const frame: TAppAudioPcmFrame = {
					...
					frameCount,
					protocolVersion,
					droppedFrameCount: typeof droppedFrameCount === 'number' ? droppedFrameCount : undefined,
					pcm: new Float32Array(pcmBuffer, byteOffset, byteLength / Float32Array.BYTES_PER_ELEMENT),
				};
		...
		const timeout = window.setTimeout(() => {
			...
			settled = true;
			ipcRenderer.removeListener('desktop:app-audio-frame-channel-ready', onPortReady);
			appAudioFramePortPromise = undefined;
			scheduleAppAudioFrameChannelReconnect();
			resolve(false);
...
const flushBeforeQuitSubscribers = async () => {
	...
	beforeQuitFlushPromise = (async () => {
		...
		const results = await Promise.allSettled(
			Array.from(beforeQuitSubscribers, (callback) => Promise.resolve().then(callback)),
		);

		const rejectedResult = results.find((result) => result.status === 'rejected');
		if (rejectedResult?.status === 'rejected') {
			console.error('[desktop] before-quit renderer flush failed', rejectedResult.reason);
			notifyMainBeforeQuitFinished({
				status: 'skipped',
...
const desktopBridge = {
	getServerUrl: (): Promise<string> => ipcRenderer.invoke('desktop:get-server-url'),
	...
	subscribeAppAudioStatus: (callback: (statusEvent: TAppAudioStatusEvent) => void) => {
	...
	subscribeGlobalPushKeybindEvents: (callback: (event: TDesktopPushKeybindEvent) => void) => {
	...
	subscribeCapabilities: (callback: (capabilities: TDesktopCapabilities) => void) => {
	...
	subscribeWindowControlsState: (callback: (state: TDesktopWindowControlsState) => void) => {
		...
		return () => {
			ipcRenderer.removeListener('desktop:window-controls-state-changed', listener);
		};
	},
