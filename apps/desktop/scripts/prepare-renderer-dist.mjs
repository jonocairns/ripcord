...
const rootFontReferencePattern = /(?:url\(\s*["']?\/fonts\/|(?:href|src)=["']\/fonts\/|["']\/fonts\/)/;
...
const verifyFontReferences = async (filePaths) => {
	for (const filePath of filePaths) {
		...
		if (rootFontReferencePattern.test(contents)) {
			throw new Error(`Root-relative font path remains in ${path.relative(cwd, filePath)}`);
		}
		...
		for (const match of contents.matchAll(fontReferencePattern)) {
			...
			const resolvedFontPath = path.resolve(path.dirname(filePath), stripAssetQuery(fontReference));

			try {
				await fs.access(resolvedFontPath);
			} catch {
				throw new Error(
					`Font reference ${fontReference} in ${path.relative(cwd, filePath)} does not resolve to a packaged file`,
				);
			}
		}
	}
...
const patchedIndexHtml = withoutFontPreload.replace(/(src|href)=["']\/([^"']+)["']/g, '$1="./$2"');
...
for (const entry of assetEntries) {
	...
	if (extension === '.css') {
		patchedContents = patchedContents.replace(/url\(\s*(["']?)\/fonts\//g, 'url($1../fonts/');
