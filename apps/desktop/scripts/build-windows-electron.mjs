...
const SIDECAR_SOURCE = path.resolve(DESKTOP_DIR, 'sidecar/target', RUST_TARGET, profile, SIDECAR_BINARY);
...
if (!existsSync(path.join(DESKTOP_DIR, 'node_modules')) || !existsSync(path.join(CLIENT_DIR, 'node_modules'))) {
...
const cargoArgs = ['build', '--manifest-path', 'sidecar/Cargo.toml', '--target', RUST_TARGET];
...
const previewConfigTarget = path.join(WINDOWS_BUILD_DIR, 'resources', PREVIEW_RUNTIME_CONFIG_FILE);
...
const rsync = spawnSync('rsync', ['-a', '--delete', `${WINDOWS_BUILD_DIR}/`, installDir], { stdio: 'inherit' });
