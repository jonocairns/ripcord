import { ActivityLogType, ChannelPermission, isEmptyMessage, Permission, toDomCommand } from '@sharkord/shared';
...
const sendMessageRoute = rateLimitedProcedure(protectedProcedure, {
	...
	.mutation(async ({ input, ctx }) => {
		...
		let editable = true;
		let commandExecutor: ((messageId: number) => void) | undefined;
		...
		if (enablePlugins) {
			...
			if (foundCommand) {
				if (await ctx.hasPermission(Permission.EXECUTE_PLUGIN_COMMANDS)) {
					...
					const plugin = await pluginManager.getPluginInfo(foundCommand?.pluginId || '');

					editable = false;
					targetContent = toDomCommand({ ...foundCommand, imageUrl: plugin?.logo, status: 'pending' }, args);

					// do not await, let it run in background
					commandExecutor = (messageId: number) => {
						const updateCommandStatus = (status: 'completed' | 'failed', response?: unknown) => {
							...
							db.update(messages).set({ content: updatedContent }).where(eq(messages.id, messageId)).execute();

							publishMessage(messageId, input.channelId, 'update');
						};

						pluginManager
							.executeCommand(foundCommand.pluginId, foundCommand.name, getInvokerCtxFromTrpcCtx(ctx), argsObject)
							.then((response) => {
