...
const restoreOrJoinVoiceRoute = rateLimitedProcedure(protectedProcedure, {
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    const reconnectLabBehavior = consumeVoiceReconnectLabNextRestoreBehavior(
      ctx.user.id
    );
    ...
    if (reconnectLabBehavior?.failCode || reconnectLabBehavior?.failMessage) {
      throw new TRPCError({
        ...
        message:
          reconnectLabBehavior.failMessage ??
          getReconnectLabFailureMessage(reconnectLabBehavior.failCode)
      });
    }
...
const isSameVoiceClientSession = (
  ...
  const candidateClientInstanceId = getTrackedWsString(
    candidateWs,
    'clientInstanceId'
  );
...
const getReconnectLabFailureMessage = (
  code: 'INTERNAL_SERVER_ERROR' | 'UNAUTHORIZED' | 'CONFLICT' | undefined
) => {
  switch (code) {
    case 'UNAUTHORIZED':
