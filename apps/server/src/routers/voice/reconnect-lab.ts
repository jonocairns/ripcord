...
const restoreBehaviorFailCodeSchema = z.enum(['INTERNAL_SERVER_ERROR', 'UNAUTHORIZED', 'CONFLICT']);
...
const nextRestoreBehaviorInputSchema = z
	...
	.refine((input) => input.closeWsReason === undefined || input.closeWsCode !== undefined, {
...
const voiceReconnectLabRouter = t.router({
	setNextRestoreBehavior: protectedProcedure.input(nextRestoreBehaviorInputSchema).mutation(({ input, ctx }) => {
