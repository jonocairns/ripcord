...
describe('messages router', () => {
	...
	test('should throw when user lacks permissions (toggleReaction)', async () => {
		...
		await caller1.roles.update({
			...
			permissions: memberRole!.permissions.filter((permission) => permission !== Permission.REACT_TO_MESSAGES),
		});
	...
	test('should edit own message', async () => {
		...
		const editedMessage = messagesAfter.messages.find((m) => m.id === messageId);
	...
	test('should delete own message', async () => {
		...
		expect(messagesAfter.messages.find((m) => m.id === messageId)).toBeUndefined();
	...
	test('should toggle reaction on message', async () => {
		...
		const messageWithReaction = messagesAfterAdd.messages.find((m) => m.id === messageId);
		...
		const messageWithoutReaction = messagesAfterRemove.messages.find((m) => m.id === messageId);
	...
	test('should allow multiple users to react to the same message', async () => {
		...
		const messageWithReactions = messagesAfter.messages.find((m) => m.id === messageId);
	...
	test('should update message updatedAt timestamp on edit', async () => {
		...
		expect(editedMessage!.updatedAt).toBeGreaterThan(originalUpdatedAt ?? editedMessage!.createdAt);
	});
