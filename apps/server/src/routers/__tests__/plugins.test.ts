...
describe('plugins router', () => {
	...
	test('should throw when user lacks permissions', async () => {
		...
		await expect(caller.plugins.get()).rejects.toThrow('Insufficient permissions');
	});
	...
	test('should include plugin metadata', async () => {
		...
		const pluginA = result.plugins.find((p: TPluginInfo) => p.id === 'plugin-a');
	...
	test('should filter out plugins with invalid package.json', async () => {
		...
		const result = await caller.plugins.get();
		const invalidPlugin = result.plugins.find((p: TPluginInfo) => p.id === 'plugin-invalid-package');
	...
	test('should load plugin when enabled', async () => {
		...
		const result = await caller.plugins.get();
		const pluginB = result.plugins.find((p: TPluginInfo) => p.id === 'plugin-b');
	...
	describe('getCommands', () => {
		...
		test('should include command metadata', async () => {
			...
			const testCommand = pluginBCommands!.find((c) => c.name === 'test-command');
