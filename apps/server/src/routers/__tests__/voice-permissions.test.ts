...
import { ChannelPermission, Permission, type TJoinedRole } from '@sharkord/shared';
...
describe('voice permission revalidation', () => {
	test('evicts active voice users when a role loses JOIN_VOICE_CHANNELS', async () => {
		...
		await ownerCaller.roles.update({
			...
			permissions: defaultRole.permissions.filter((permission) => permission !== Permission.JOIN_VOICE_CHANNELS),
		});

		expect(runtime.getUser(2)).toBeUndefined();

		await expect(userCaller.voice.createProducerTransport()).rejects.toThrow('Insufficient permissions');
	});
	...
	test('evicts active voice users when private-channel access is revoked', async () => {
		...
		await expect(userCaller.voice.createProducerTransport()).rejects.toThrow('User is not in a voice channel');
	});
