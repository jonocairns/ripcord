...
const deleteCategoryRoute = protectedProcedure
	...
	.mutation(async ({ input, ctx }) => {
		...
		const removedCategory = await db.delete(categories).where(eq(categories.id, input.categoryId)).returning().get();

		invariant(removedCategory, 'Category not found');

		await db.delete(channels).where(eq(channels.categoryId, removedCategory.id));
