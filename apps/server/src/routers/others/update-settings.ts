...
const updateSettingsRoute = protectedProcedure
	...
	.mutation(async ({ input, ctx }) => {
		...
		const nextPasswordInput = input.password === undefined ? undefined : input.password ? input.password : null;
		const nextPassword =
			typeof nextPasswordInput === 'string' ? await hashPassword(nextPasswordInput) : nextPasswordInput;
