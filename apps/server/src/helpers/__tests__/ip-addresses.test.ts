...
describe('ip-addresses helpers', () => {
	...
	test('resolves preferred addresses with fallback', () => {
		expect(resolvePreferredAddress({ ipv4: '198.51.100.10', ipv6: '2001:db8::10' }, 'ipv6')).toBe('2001:db8::10');
		expect(resolvePreferredAddress({ ipv4: '198.51.100.10' }, 'ipv6')).toBe('198.51.100.10');
	});
