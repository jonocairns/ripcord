...
describe('parseCommandArgs', () => {
	...
	test('handles mixed quoted and unquoted arguments', () => {
		expect(parseCommandArgs('/warn user123 "first reason" second "third reason"')).toEqual({
			commandName: 'warn',
	...
	test('parses command with URL argument', () => {
		expect(parseCommandArgs('/play https://www.youtube.com/watch?v=abc123XYZ')).toEqual({
			commandName: 'play',
