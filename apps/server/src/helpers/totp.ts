...
const generateTotpSetup = async (identity: string, serverName: string): Promise<TTotpSetupResult> => {
 ...
 * the validity window.
 */
const verifyAndConsumeTotpToken = (userId: number, secret: string, token: string): boolean => {
...
const verifyRecoveryCode = async (
	...
	const remainingCodes = [...hashedCodes.slice(0, matchIndex), ...hashedCodes.slice(matchIndex + 1)];
...
const verifyChallengeToken = async (token: string): Promise<TChallengePayload | null> => {
	try {
		const jwt = await import('jsonwebtoken');
		const serverToken = await getServerToken();
		const decoded = zChallengePayload.parse(jwt.default.verify(token, serverToken));
