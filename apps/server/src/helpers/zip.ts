...
const unzipBlobToDirectory = async (zipBlob: Blob | Buffer | Uint8Array, outDir: string): Promise<void> => {
		...
		yauzl.fromBuffer(buffer, { lazyEntries: true }, (err, zipfile) => {
			...
			zipfile.on('entry', (entry) => {
				...
				if (!entryPath.startsWith(path.resolve(outDir))) {
					return reject(new Error(`Illegal file path in zip: ${entry.fileName}`));
				}
