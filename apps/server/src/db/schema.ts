import type { TActivityLogDetailsMap, TMessageMetadata, TUserPresenceStatus } from '@sharkord/shared';
...
const settings = sqliteTable(
	...
	(t) => [index('settings_server_idx').on(t.serverId), uniqueIndex('settings_server_unique_idx').on(t.serverId)],
);
...
const roles = sqliteTable(
	...
	(t) => [index('roles_is_default_idx').on(t.isDefault), index('roles_is_persistent_idx').on(t.isPersistent)],
);
...
const users = sqliteTable(
	'users',
	{
		...
		presenceStatus: text('presence_status').$type<TUserPresenceStatus>().notNull().default('online'),
...
const activityLog = sqliteTable(
	'activity_log',
	{
		...
		type: text('type').notNull(),
		details: text('details', { mode: 'json' }).$type<TActivityLogDetailsMap[keyof TActivityLogDetailsMap]>(),
		ip: text('ip'),
		createdAt: integer('created_at').notNull(),
	},
	(t) => [
...
const channelRolePermissions = sqliteTable(
	...
	(t) => [
		...
		index('channel_role_permissions_channel_perm_idx').on(t.channelId, t.permission),
...
const channelUserPermissions = sqliteTable(
	...
	(t) => [
		...
		index('channel_user_permissions_channel_perm_idx').on(t.channelId, t.permission),
