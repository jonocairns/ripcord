...
type TEmojiRow = {
	...
	user: Pick<
		TJoinedEmoji['user'],
		'id' | 'name' | 'bannerColor' | 'bio' | 'createdAt' | 'banned' | 'avatarId' | 'bannerId'
	>;
};
...
const emojiExists = async (name: string): Promise<boolean> => {
	const emoji = await db.select().from(emojis).where(eq(emojis.name, name)).limit(1).get();
...
const getEmojiFileIdByEmojiName = async (name: string): Promise<number | null> => {
...
export { emojiExists, getEmojiById, getEmojiFileIdByEmojiName, getEmojis, getUniqueEmojiName };
