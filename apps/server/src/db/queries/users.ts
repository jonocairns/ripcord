...
const getPublicUserById = async (userId: number): Promise<TJoinedPublicUser | undefined> => {
	...
	const roles = await db.select({ roleId: userRoles.roleId }).from(userRoles).where(eq(userRoles.userId, userId)).all();
...
const getPublicUsers = async (returnIdentity: boolean = false): Promise<TJoinedPublicUser[]> => {
	const avatarFiles = alias(files, 'avatarFiles');
	const bannerFiles = alias(files, 'bannerFiles');
...
const getStorageUsageByUserId = async (userId: number): Promise<TStorageData> => {
...
const getUserByIdentity = async (identity: string): Promise<TJoinedUser | undefined> => {
	const avatarFiles = alias(files, 'avatarFiles');
	const bannerFiles = alias(files, 'bannerFiles');
...
const getUserByToken = async (token: string | undefined) => {
	try {
		...
		const decoded = zTokenPayload.parse(jwt.verify(token, await getServerToken()));
...
const getUserIdsWithPermission = async (permission: Permission): Promise<number[]> => {
	const rows = await db
		.select({
		...
		.where(or(eq(userRoles.roleId, OWNER_ROLE_ID), eq(rolePermissions.permission, permission)))
