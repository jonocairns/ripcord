...
const getSettings = async (): Promise<TJoinedSettings> => {
	...
	const logo = serverSettings.logoId
		? await db.select().from(files).where(eq(files.id, serverSettings.logoId)).get()
		: undefined;
...
type TRedactedSettings = Omit<TJoinedSettings, 'authTokenSecret' | 'password' | 'secretToken'> & {
	hasPassword: boolean;
};

const redactSettings = (serverSettings: TJoinedSettings): TRedactedSettings => {
	const { authTokenSecret: _authTokenSecret, password, secretToken: _secretToken, ...safeSettings } = serverSettings;
...
const getServerToken = async (): Promise<string> => {
	...
	const serverSettings = await db.select({ authTokenSecret: settings.authTokenSecret }).from(settings).get();
	...
	if (!serverSettings.authTokenSecret) {
		...
		await db.update(settings).set({ authTokenSecret: generatedAuthToken }).where(isNotNull(settings.serverId)).run();
...
export { getPublicSettings, getServerToken, getServerTokenSync, getSettings, redactSettings };
