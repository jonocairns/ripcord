...
import { categories, channels, messages, rolePermissions, roles, settings, userRoles, users } from '../db/schema';
...
const seedDatabase = async (db: BunSQLiteDatabase) => {
	...
	const [insertedDefaultRole] = await db.insert(roles).values(defaultRole).returning();
