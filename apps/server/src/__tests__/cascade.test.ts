...
describe('database cascades', async () => {
	...
	test('deleting a category cascades to channels and messages', async () => {
		...
		const channelsInCategory = channelsBefore.filter((ch) => ch.categoryId === categoriesBefore[0]!.id);

		await tdb.delete(categories).where(eq(categories.id, categoriesBefore[0]!.id));

		const channelsAfter = await tdb.select().from(channels);
		expect(channelsAfter.length).toBe(channelsBefore.length - channelsInCategory.length);
	...
	test('deleting a user cascades to user_roles', async () => {
		...
		const userRolesBefore = await tdb.select().from(userRoles).where(eq(userRoles.userId, userId));

		expect(userRolesBefore.length).toBeGreaterThan(0);

		await tdb.delete(users).where(eq(users.id, userId));

		const userRolesAfter = await tdb.select().from(userRoles).where(eq(userRoles.userId, userId));
	...
	test('deleting a role cascades to user_roles and role_permissions', async () => {
		...
		const rolePermissionsBefore = await tdb.select().from(rolePermissions).where(eq(rolePermissions.roleId, roleId));

		const userRolesBefore = await tdb.select().from(userRoles).where(eq(userRoles.roleId, roleId));
		...
		const rolePermissionsAfter = await tdb.select().from(rolePermissions).where(eq(rolePermissions.roleId, roleId));

		const userRolesAfter = await tdb.select().from(userRoles).where(eq(userRoles.roleId, roleId));
	...
	test('deleting a user cascades to channel_read_states and channel_user_permissions', async () => {
		...
		const readStatesBefore = await tdb.select().from(channelReadStates).where(eq(channelReadStates.userId, userId));
		...
		const readStatesAfter = await tdb.select().from(channelReadStates).where(eq(channelReadStates.userId, userId));
	...
	test('deleting a message cascades to message_files and message_reactions', async () => {
		...
		const messageFilesBefore = await tdb.select().from(messageFiles).where(eq(messageFiles.messageId, messageId));
		...
		const messageFilesAfter = await tdb.select().from(messageFiles).where(eq(messageFiles.messageId, messageId));
	...
	test('deleting a file cascades to message_files and emojis', async () => {
		...
		const messageFilesBefore = await tdb.select().from(messageFiles).where(eq(messageFiles.fileId, file!.id));

		const emojisBefore = await tdb.select().from(emojis).where(eq(emojis.fileId, file!.id));
		...
		const messageFilesAfter = await tdb.select().from(messageFiles).where(eq(messageFiles.fileId, file!.id));

		const emojisAfter = await tdb.select().from(emojis).where(eq(emojis.fileId, file!.id));
	...
	test('deleting a user cascades to emojis and message_reactions', async () => {
		...
		const emojisBefore = await tdb.select().from(emojis).where(eq(emojis.userId, userId));

		const messageReactionsBefore = await tdb.select().from(messageReactions).where(eq(messageReactions.userId, userId));
		...
		const emojisAfter = await tdb.select().from(emojis).where(eq(emojis.userId, userId));

		const messageReactionsAfter = await tdb.select().from(messageReactions).where(eq(messageReactions.userId, userId));
