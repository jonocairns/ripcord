...
const parseRange = (header: string, totalSize: number): TParsedRange | 'unsatisfiable' | null => {
...
const publicRouteHandler = async (req: http.IncomingMessage, res: http.ServerResponse) => {
	if (!req.url) {
	...
	const dbFile = await db.select().from(files).where(eq(files.name, fileName)).get();
	...
	if (associatedMessage) {
		...
		if (channel?.private) {
			...
			const isValidToken = verifyFileToken(dbFile.id, channel.fileAccessToken, accessToken || '');
	...
	const contentDisposition = INLINE_ALLOWLIST.has(dbFile.mimeType) ? 'inline' : 'attachment';
