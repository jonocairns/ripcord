...
describe('/interface', () => {
	...
	beforeAll(() => {
		...
		const fileWithSpaces = path.join(testInterfacePath, 'test file with spaces.html');
...
<body>
  ...
  <script type="module" src="/assets/test.js"></script>
	...
	test('should serve JavaScript files with correct content type', async () => {
		...
		expect(contentType?.includes('javascript') || contentType?.includes('text/plain')).toBe(true);
	...
	test('should prevent encoded path traversal attacks', async () => {
		const response = await fetch(`${testsBaseUrl}/${encodeURIComponent('../../../etc/passwd')}`);
	...
	test('should handle query parameters in URLs', async () => {
		const response = await fetch(`${testsBaseUrl}/index.html?v=123&cache=false`);
