...
const verify2faRouteHandler = async (req: http.IncomingMessage, res: http.ServerResponse) => {
	...
	if (!totpData?.totpSecret) {
		throw new HttpValidationError('code', 'Two-factor authentication is not enabled for this account');
	}
	...
	if (data.isRecoveryCode) {
		...
		if (totpData.totpRecoveryCodes) {
			try {
				storedCodes = zRecoveryCodes.parse(JSON.parse(totpData.totpRecoveryCodes));
			} catch {
				throw new HttpValidationError('code', 'Recovery codes are unavailable for this account');
			}
		}

		const { valid, remainingCodes } = await verifyRecoveryCode(data.code, storedCodes);
		...
		const isValid = verifyAndConsumeTotpToken(user.id, decryptedSecret, data.code);
	...
	const { token, refreshToken } = await issueAuthTokens(user.id, user.tokenVersion);
