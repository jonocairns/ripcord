...
const printDebug = () => {
	...
	const message = [
		...
		`${chalk.blue('Local addresses:')} ${chalk.bold(formatResolvedIpAddresses(SERVER_PRIVATE_IPS))}`,
		`${chalk.blue('Public addresses:')} ${chalk.bold(formatResolvedIpAddresses(SERVER_PUBLIC_IPS))}`,
		...
		`${chalk.blue('Config:')} ${chalk.bold(String(JSON.stringify(config, null, 2)))}`,
	...
	console.log(chalk.white.bold('Debug mode is enabled. This may affect performance.'));
};
