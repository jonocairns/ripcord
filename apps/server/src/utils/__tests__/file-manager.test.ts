...
describe('file manager', () => {
	...
	test('should throw error for non-existent temporary file', async () => {
		await expect(fileManager.removeTemporaryFile('non-existent-id')).rejects.toThrow('Temporary file not found');
	});
	...
	test('should insert file record in database', async () => {
		...
		const dbFile = await tdb.select().from(files).where(eq(files.id, savedFile.id)).get();
	...
	test('should throw error when saving non-existent temporary file', async () => {
		await expect(fileManager.saveFile('non-existent-id', 1)).rejects.toThrow('File not found');
	});
	...
	test('should generate sequential file IDs', async () => {
		...
		tempFilesToCleanup.push(path.join(PUBLIC_PATH, savedFile1.name), path.join(PUBLIC_PATH, savedFile2.name));
	...
	test('should throw error when user storage limit exceeded', async () => {
		...
		await expect(fileManager.saveFile(tempFile.id, 1)).rejects.toThrow('User storage limit exceeded');
	...
	test('should throw error when server storage limit exceeded with PREVENT_UPLOADS', async () => {
		...
		await expect(fileManager.saveFile(tempFile.id, 1)).rejects.toThrow('Server storage limit exceeded.');
	...
	test('should delete old files when storage limit exceeded with DELETE_OLD_FILES', async () => {
		...
		const oldDbFile = await tdb.select().from(files).where(eq(files.id, oldSavedFile.id)).get();

		expect(oldDbFile).toBeUndefined();

		const newDbFile = await tdb.select().from(files).where(eq(files.id, newSavedFile.id)).get();
