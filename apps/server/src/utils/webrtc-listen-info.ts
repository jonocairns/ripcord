...
const getDefaultBindAddress = (family: TIpFamily, isProduction: boolean): string => {
	if (isProduction) {
...
const getFamilyConfig = (config: TWebRtcConfig, family: TIpFamily): TWebRtcFamilyConfig => {
...
const sortListenInfosByPreference = (listenInfos: TWebRtcListenInfo[], preferredFamily: TIpFamily) => {
...
const buildWebRtcListenInfos = (
	...
	for (const family of ['ipv4', 'ipv6'] satisfies TIpFamily[]) {
		...
		const defaultBindAddress = getDefaultBindAddress(family, options.isProduction);
		const bindAddress = options.isProduction
			? familyConfig.bindAddress || defaultBindAddress
			: !familyConfig.bindAddress || isUnspecifiedBindAddress(familyConfig.bindAddress)
				? defaultBindAddress
		...
		if (options.isProduction && isUnspecifiedBindAddress(bindAddress) && !announcedAddress) {
			continue;
		}

		listenInfos.push(...getListenInfoProtocolEntries(family, bindAddress, config.port, announcedAddress));
	}
