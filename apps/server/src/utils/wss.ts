...
const hasOtherOpenUserVoiceConnection = (userId: number, currentWs: TTrackedWebSocket, channelId: number) => {
	return getTrackedClients().some(
		(client) =>
			client !== currentWs &&
...
const hasAnyOpenUserVoiceConnection = (userId: number, channelId: number) => {
	return getTrackedClients().some(
		(client) =>
			client.userId === userId && client.readyState === WebSocket.OPEN && client.currentVoiceChannelId === channelId,
	);
};
...
const createContext = async ({ info, req, res }: CreateWSSContextFnOptions): Promise<Context> => {
	...
	const hasChannelPermission = async (channelId: number, targetPermission: ChannelPermission) => {
		...
		const userChannelPermissions = await getAllChannelUserPermissions(decodedUser.id);
	...
	const getStatusById = (userId: number) => {
		...
		const isAway = userConnections.some((ws) => ws.presenceStatus === UserStatus.AWAY);
	...
	const setWsVoiceChannelId = (channelId: number | undefined) => {
		...
		const ws = getTrackedClients().find((client) => isCurrentClient(client) && client.userId === decodedUser.id);
	...
	const needsChannelPermission = async (channelId: number, targetPermission: ChannelPermission) => {
	...
	return new Promise<TWsServer>((resolve) => {
		...
		wss.on('connection', (ws) => {
			...
			trackedWs.on('close', async (wsCloseCode) => {
				...
				if (trackedWs.currentVoiceChannelId !== undefined && !hasOtherVoiceConnection) {
				...
				if (hasOtherConnections) {
					logger.debug('User %s disconnected from one session, but is still connected elsewhere', userId);
					return;
				}
