...
class TemporaryFileManager {
	...
	private pruneMissingTemporaryFiles = async (): Promise<void> => {
		...
		this.temporaryFiles = existingFiles.filter((file): file is TTempFile => file !== undefined);
	};

	private assertWithinTemporaryCapacity = async (size: number, userId: number): Promise<void> => {
		...
		if (totalFileCount >= TEMP_MAX_FILES_GLOBAL || userFileCount >= TEMP_MAX_FILES_PER_USER) {
			throw new TemporaryFileCapacityError('Too many temporary uploads in progress');
		}
		...
		if (totalBytes + size > globalByteLimit || userBytes + size > userByteLimit) {
			throw new TemporaryFileCapacityError('Temporary upload storage limit exceeded');
		}
	};
	...
	public removeTemporaryFile = async (id: string, skipDelete = false): Promise<void> => {
	...
	private cleanupDirectory = async (dir: string, maxAgeMs: number): Promise<void> => {
		let entries: string[];
		try {
...
class FileManager {
	...
	private handleStorageLimits = async (tempFile: TTempFile) => {
		...
		if (settings.storageSpaceQuotaByUser > 0 && newTotalStorage > settings.storageSpaceQuotaByUser) {
		...
		if (settings.storageQuota > 0 && newServerStorage > settings.storageQuota) {
			if (settings.storageOverflowAction === StorageOverflowAction.PREVENT_UPLOADS) {
				throw new Error('Server storage limit exceeded.');
			}

			if (settings.storageOverflowAction === StorageOverflowAction.DELETE_OLD_FILES) {
