...
class PluginManager {
	...
	private setPluginEnabled = async (pluginId: string, enabled: boolean) => {
		...
		await db.insert(pluginData).values({ pluginId, enabled }).onConflictDoUpdate({
			target: pluginData.pluginId,
			set: { enabled },
		});
	};
	...
	public loadPlugins = async () => {
		...
		for (const file of files) {
			try {
				...
				logger.error(`Failed to load plugin ${file}: ${(error as Error).message}`);
			}
		}
	};
	...
	public unloadPlugins = async () => {
		for (const pluginId of this.loadedPlugins.keys()) {
			try {
				...
				logger.error(`Failed to unload plugin ${pluginId}: ${(error as Error).message}`);
			}
		}
	};
	...
	private logPlugin = (pluginId: string, type: 'info' | 'error' | 'debug', ...message: unknown[]) => {
		if (!this.logs.has(pluginId)) {
			this.logs.set(pluginId, []);
		}

		const loggerFn = logger[type];
		const parsedMessage = message.map((m) => (typeof m === 'object' ? JSON.stringify(m) : String(m))).join(' ');
	...
	private unregisterPluginCommands = (pluginId: string) => {
		...
		this.logPlugin(pluginId, 'debug', `Unregistered ${commandNames.length} command(s): ${commandNames.join(', ')}`);
	};
	...
	public executeCommand = async <TArgs = unknown>(
		...
		if (!foundCommand) {
			throw new Error(`Command '${commandName}' not found for plugin '${pluginId}'.`);
		}

		try {
			this.logPlugin(pluginId, 'debug', `Executing command '${commandName}' with args:`, args);

			return await foundCommand.command.executes(invokerCtx, args);
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : String(error);

			this.logPlugin(pluginId, 'error', `Error executing command '${commandName}': ${errorMessage}`);
	...
	public unload = async (pluginId: string) => {
		...
		if (!pluginModule) {
			this.logPlugin(pluginId, 'debug', `Plugin ${pluginId} is not loaded; nothing to unload.`);
			return;
		}
	...
	public getPluginInfo = async (pluginId: string): Promise<TPluginInfo> => {
		...
		const packageJson = zPluginPackageJson.parse(JSON.parse(await fs.readFile(packageJsonPath, 'utf-8')));
	...
	public load = async (pluginId: string) => {
		...
		if (!this.isPluginEnabled(pluginId)) {
			this.logPlugin(pluginId, 'debug', `Plugin ${pluginId} is disabled; skipping load.`);
			return;
		}
		...
		try {
			...
			if (typeof mod.onLoad !== 'function') {
				throw new Error(`Plugin ${pluginId} does not export an 'onLoad' function`);
			}
			...
			this.logPlugin(pluginId, 'info', `Plugin loaded: ${pluginId}@v${info.version} by ${info.author}`);
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : String(error);

			this.loadErrors.set(pluginId, errorMessage);

			this.logPlugin(pluginId, 'error', `Failed to load plugin ${pluginId}: ${errorMessage}`);

			await this.unload(pluginId);
		}
	};

	private loadSettingsFromDb = async (pluginId: string): Promise<Record<string, unknown>> => {
		const rows = await db.select().from(pluginData).where(eq(pluginData.pluginId, pluginId));
	...
	private saveSettingsToDb = async (pluginId: string, values: Record<string, unknown>) => {
	...
	private registerSettings = async (
		...
		for (const def of definitions) {
			merged[def.key] = dbValues[def.key] !== undefined ? dbValues[def.key] : def.defaultValue;
		}
		...
		return {
			...
			set: (key: string, value: unknown) => {
				...
				const def = this.settingDefinitions.get(pluginId)?.find((d) => d.key === key);
				if (!def) {
					this.logPlugin(pluginId, 'error', `Setting key '${key}' is not registered.`);
					return;
				}
				...
				this.saveSettingsToDb(pluginId, values).catch((err) => {
					this.logPlugin(pluginId, 'error', `Failed to persist setting '${key}':`, err);
				});
			},
		};
	};

	public getPluginSettings = async (pluginId: string): Promise<TPluginSettingsResponse> => {
		...
		if (!values) {
			...
			for (const def of definitions) {
				values[def.key] = dbValues[def.key] !== undefined ? dbValues[def.key] : def.defaultValue;
			}
		}
	...
	public updatePluginSetting = async (pluginId: string, key: string, value: unknown) => {
		const definitions = this.settingDefinitions.get(pluginId);
		...
		return {
			...
			actions: {
				voice: {
					...
					createStream: (options: TCreateStreamOptions): TExternalStreamHandle => {
						...
						if (!channel) {
							throw new Error(`Voice runtime not found for channel ID ${options.channelId}`);
						}
						...
						if (options.producers.audio) {
							pubsub.publishForChannel(options.channelId, ServerEvents.VOICE_NEW_PRODUCER, {
								channelId: options.channelId,
								remoteId: streamId,
			...
			commands: {
				register: <TArgs = void>(command: CommandDefinition<TArgs>) => {
					...
					const existingIndex = pluginCommands.findIndex((c) => c.name === command.name);

					if (existingIndex !== -1) {
						this.logPlugin(pluginId, 'error', `Command '${command.name}' is already registered. Overwriting.`);
			...
			settings: {
				register: (definitions) => {
					return this.registerSettings(pluginId, definitions) as ReturnType<PluginContext['settings']['register']>;
				},
			},
		};
	};
