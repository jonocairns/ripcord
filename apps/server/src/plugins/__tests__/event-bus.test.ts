...
describe('event-bus', () => {
	...
	describe('register', () => {
		test('should register a handler for a plugin', () => {
			...
			expect(eventBus.getPluginHandlersCount('plugin1', 'message:created')).toBe(1);
		});
		...
		test('should register multiple handlers for the same event', () => {
			...
			expect(eventBus.getPluginHandlersCount('plugin1', 'message:created')).toBe(2);
		});
		...
		test('should register handlers from different plugins for the same event', () => {
			...
			expect(eventBus.getListenersCount('message:created')).toBe(2);
			expect(eventBus.getPluginHandlersCount('plugin1', 'message:created')).toBe(1);
			expect(eventBus.getPluginHandlersCount('plugin2', 'message:created')).toBe(1);
		});
