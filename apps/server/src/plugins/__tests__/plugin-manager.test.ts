...
describe('plugin-manager', () => {
	...
	describe('load', () => {
		...
		test('should load plugin-b with commands', async () => {
			...
			const hasTestCommand = pluginManager.hasCommand('plugin-b', 'test-command');
		...
		test('should skip loading disabled plugin', async () => {
			...
			const hasSkipMessage = logs.some((log) => log.message.includes('skipping load'));
		...
		test('should reject when plugins are disabled in settings', async () => {
			...
			await expect(pluginManager.load('plugin-a')).rejects.toThrow('Plugins are disabled.');
		});

		test('should handle plugin with invalid package.json', async () => {
			await expect(pluginManager.getPluginInfo('plugin-invalid-package')).rejects.toThrow();
		});

		test('should handle plugin with missing entry file', async () => {
			await expect(pluginManager.getPluginInfo('plugin-missing-entry')).rejects.toThrow('Plugin entry file not found');
		});
	...
	describe('unload', () => {
		test('should unload plugin-a correctly', async () => {
			...
			const hasUnloadMessage = logs.some((log) => log.message.includes('unloaded'));
	...
	describe('commands', () => {
		test('should execute command successfully', async () => {
			...
			const result = await pluginManager.executeCommand('plugin-b', 'sum', mockInvokerCtx, {
				a: 5,
				b: 3,
			});
		...
		test('should execute command with string argument', async () => {
			...
			const result = await pluginManager.executeCommand('plugin-b', 'test-command', mockInvokerCtx, {
				message: 'Hello World',
			});
		...
		test('should throw error when plugin has no commands', async () => {
			...
			await expect(pluginManager.executeCommand('plugin-a', 'nonexistent', mockInvokerCtx, {})).rejects.toThrow(
		...
		test('should throw error when command does not exist', async () => {
			...
			await expect(pluginManager.executeCommand('plugin-b', 'nonexistent', mockInvokerCtx, {})).rejects.toThrow(
				'not found',
			);
		});
	...
	describe('getPluginInfo', () => {
		test('should return correct plugin info', async () => {
			...
			expect(info.description).toBe('This is a mocked plugin for testing purposes.');
		...
		test('should throw error for non-existent plugin', async () => {
			await expect(pluginManager.getPluginInfo('nonexistent-plugin')).rejects.toThrow('package.json not found');
		});
	});
	...
	describe('command execution error handling', () => {
		test('should propagate error when command throws', async () => {
			...
			const hasDebugLog = logsAfter
				...
				.some((log) => log.type === 'debug' && log.message.includes('Executing command'));
	...
	describe('plugin ID validation', () => {
		test('should reject plugin ID with path traversal', async () => {
			await expect(pluginManager.getPluginInfo('../../../etc')).rejects.toThrow('Invalid plugin ID');
		});

		test('should reject plugin ID with forward slash', async () => {
			await expect(pluginManager.getPluginInfo('foo/bar')).rejects.toThrow('Invalid plugin ID');
		});
		...
		test('should reject plugin ID with null byte', async () => {
			await expect(pluginManager.getPluginInfo('foo\0bar')).rejects.toThrow('Invalid plugin ID');
		});
	});
	...
	describe('settings', () => {
		test('should register settings and return default values', async () => {
			...
			const result = await pluginManager.executeCommand('plugin-with-settings', 'get-settings', mockInvokerCtx, {});
		...
		test('should update settings via plugin command', async () => {
			...
			await pluginManager.executeCommand('plugin-with-settings', 'set-greeting', mockInvokerCtx, { value: 'Welcome!' });
		...
		test('should return settings definitions via getPluginSettings', async () => {
			...
			const settings = await pluginManager.getPluginSettings('plugin-with-settings');
		...
		test('should update settings via updatePluginSetting', async () => {
			...
			await pluginManager.updatePluginSetting('plugin-with-settings', 'maxRetries', 5);
		...
		test('should throw error when updating unregistered setting key', async () => {
			...
			await expect(pluginManager.updatePluginSetting('plugin-with-settings', 'nonexistent', 'value')).rejects.toThrow(
				'not registered',
			);
		});
		...
		test('should throw error when plugin has no settings', async () => {
			...
			await expect(pluginManager.updatePluginSetting('plugin-a', 'key', 'value')).rejects.toThrow(
		...
		test('should persist settings to DB and restore on reload', async () => {
			...
			// update a setting
			await pluginManager.updatePluginSetting('plugin-with-settings', 'greeting', 'Persisted!');
		...
		test('should clean up in-memory settings on unload', async () => {
			...
			const settingsBefore = await pluginManager.getPluginSettings('plugin-with-settings');

			expect(settingsBefore.definitions).toHaveLength(3);

			await pluginManager.unload('plugin-with-settings');

			const settingsAfter = await pluginManager.getPluginSettings('plugin-with-settings');
	...
	describe('event bus integration', () => {
		...
		test('should fire event handlers when events are emitted', async () => {
			...
			const result = await pluginManager.executeCommand('plugin-with-events', 'get-counts', mockInvokerCtx, {});
		...
		test('should not fire events after plugin is unloaded', async () => {
			...
			// get count
			const result1 = await pluginManager.executeCommand('plugin-with-events', 'get-counts', mockInvokerCtx, {});
