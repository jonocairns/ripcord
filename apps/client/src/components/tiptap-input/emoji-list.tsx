...
const EmojiList = forwardRef<EmojiListRef, EmojiListProps>(({ items, onSelect }, ref) => {
			...
			{items.map((item, index) => (
				...
				>
					{item.emoji ? (
						...
						<img src={item.fallbackImage} alt={item.name} className="size-4 shrink-0 rounded-sm" />
					) : null}
