...
type TVoiceSurfaceProps = ComponentPropsWithoutRef<'div'> &
  VariantProps<typeof voiceSurfaceVariants>;

const VoiceSurface = forwardRef<HTMLDivElement, TVoiceSurfaceProps>(
  ({ className, variant, clip, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(voiceSurfaceVariants({ variant, clip }), className)}
        {...props}
      />
    );
  }
