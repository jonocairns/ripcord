import { cloneElement, isValidElement, memo, type ReactNode, useMemo } from 'react';
...
const VoiceGrid = memo(({ children, pinnedCardId, className }: TVoiceGridProps) => {
	const { gridCols, pinnedCard, regularCards } = useMemo(() => {
		...
		if (pinnedCardId) {
			const pinned = childArray.find((child: ReactNode) => isValidElement(child) && child.key === pinnedCardId);

			const regular = childArray.filter((child: ReactNode) => !isValidElement(child) || child.key !== pinnedCardId);
	...
	return (
		<div
			className={cn('grid h-full gap-3 p-3 pb-28 md:gap-4 md:p-4 md:pb-32', className)}
			style={{
