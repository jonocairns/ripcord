		...
		<OverrideLayout>
			<div className="flex gap-3 rounded-lg border border-border bg-muted/50 px-3 py-2.5">
				<div className="flex size-8 shrink-0 items-center justify-center rounded text-primary">
					{command.logo ? (
						<img src={command.logo} alt="" className="size-5 rounded object-cover" />
					) : (
				...
				<div className="flex min-w-0 flex-1 flex-col gap-1.5">
					<div className="flex items-center gap-2">
						<span className="text-sm font-semibold text-foreground">{command.commandName}</span>
					...
					<div className="flex items-center gap-2 text-xs text-muted-foreground">{command.pluginId}</div>

					{command.args.length > 0 && (
						<div className="mt-0.5 flex flex-col gap-1 rounded-md bg-background/50 px-2 py-1.5">
							{command.args.map((arg, index) => (
								<div key={index} className="flex items-baseline gap-2">
									<span className="shrink-0 text-xs font-medium text-muted-foreground">{arg.name}:</span>
									<span className="truncate font-mono text-xs text-foreground">{formatValue(arg.value)}</span>
								</div>
							))}
						</div>
					)}
