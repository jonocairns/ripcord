...
const Message = memo(({ message, actionsVisible, onHover, anchorToGroup }: TMessageProps) => {
	const [isEditing, setIsEditing] = useState(false);
	const isFromOwnUser = useIsOwnUser(message.userId);
	const can = useCan();

	const canManage = useMemo(() => can(Permission.MANAGE_MESSAGES) || isFromOwnUser, [can, isFromOwnUser]);
	...
	return (
		...
		>
			{!isEditing ? (
				...
				<MessageEditInline message={message} onBlur={() => setIsEditing(false)} />
			)}
		</div>
	);
});
