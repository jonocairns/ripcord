import { Permission, type TFile, type TJoinedMessageReaction } from '@sharkord/shared';
...
const MessageReactions = memo(({ messageId, reactions }: TMessageReactionsProps) => {
	...
	const aggregatedReactions = useMemo((): TAggregatedReaction[] => {
		...
		return Array.from(reactionMap.values()).sort((a, b) => b.createdAt + a.createdAt);
	}, [reactions, ownUserId]);
	...
	return (
		<div className="mt-1 flex flex-wrap gap-1">
			{aggregatedReactions.map((reaction) => {
				const tooltipContent = reaction.userIds.map((userId) => usernames[userId] || 'Unknown').join(', ');

				return (
					<Tooltip content={tooltipContent} key={`reaction-${reaction.emoji}`}>
						<Button
							size="sm"
							variant="outline"
							onClick={() => handleReactionClick(reaction.emoji)}
							disabled={!can(Permission.REACT_TO_MESSAGES)}
							className={cn('flex items-center gap-1', reaction.isUserReacted ? 'border-border' : 'border-none')}
						>
