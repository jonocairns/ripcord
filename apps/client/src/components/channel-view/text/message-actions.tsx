...
const MessageActions = memo(
	({ onEdit, messageId, canManage, editable, visible, anchorToGroup }: TMessageActionsProps) => {
		const { recentEmojis } = useRecentEmojis();
		const recentEmojisToShow = useMemo(() => recentEmojis.slice(0, MAX_QUICK_EMOJIS), [recentEmojis]);
						...
						{recentEmojisToShow.map((emoji) => (
							...
							>
								{emoji.emoji ? (
									...
									<img src={emoji.fallbackImage} alt={emoji.name} className="h-5 w-5 object-contain" />
								)}
							</button>
						))}
