...
import { useVoiceTransportStats } from '@/features/server/voice/hooks';
...
// Subscribes to the 1 Hz transport-stats store, so it is only mounted while
// the popover is open — closed popovers must not re-render on every sample.
					...
					<div className="space-y-2 text-muted-foreground">
						{producer.outboundVideo.map((video, index) => (
							<div key={video.id} className="space-y-0.5">
								...
								{video.configuredMaxBitrate !== null && <div>Max: {formatBitrate(video.configuredMaxBitrate)}</div>}
