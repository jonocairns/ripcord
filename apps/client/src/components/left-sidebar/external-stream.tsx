...
const ExternalStream = memo(({ title, tracks, pluginId, avatarUrl }: TExternalStreamProps) => {
	const hasVideo = tracks?.video;
	const hasAudio = tracks?.audio;

	return (
		<div className="flex items-center gap-3 rounded px-3 py-2 text-sm">
			<Tooltip content={pluginId ? `External Stream (${pluginId})` : 'External Stream'}>
				{avatarUrl ? (
					<img src={avatarUrl} alt={title} className="h-5 w-5 rounded object-cover" />
				) : (
