...
import { ChevronUp, HeadphoneOff, Headphones, Mic, MicOff, Volume2, VolumeX } from 'lucide-react';
...
import { MASTER_OUTPUT_VOLUME_KEY, OWN_MIC_VOLUME_KEY } from '../voice-provider/volume-control-storage';
...
const VolumeControlButton = memo(({ className, onChange, title, volume }: TVolumeControlButtonProps) => {
	...
	return (
		<Popover>
			...
			<PopoverContent side="top" align="center" className="w-44 p-3" onClick={(event) => event.stopPropagation()}>
	...
	return (
		<div className="flex items-center gap-1">
			<div className="flex items-center">
				...
				>
					{ownVoiceState.micMuted ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}
				</Button>

				<VolumeControlButton onChange={handleMicVolumeChange} title="Microphone volume" volume={micVolume} />
			</div>
			...
			<div className="flex items-center">
				...
				<VolumeControlButton onChange={handleOutputVolumeChange} title="Speaker volume" volume={outputVolume} />
			</div>
		</div>
	);
});
