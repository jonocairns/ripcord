...
import { useCategories, useCategoryById } from '@/features/server/categories/hooks';
...
const Category = memo(({ categoryId }: TCategoryProps) => {
	...
	return (
		<div
			...
			className="mb-4"
		>
			<div className="mb-1 flex w-full items-center px-2 py-1 text-[11px] font-semibold tracking-widest text-muted-foreground/40 uppercase">
				...
				<Protect permission={Permission.MANAGE_CHANNELS}>
					<IconButton variant="ghost" size="sm" icon={Plus} onClick={onCreateChannelClick} title="Create channel" />
				</Protect>
			</div>
