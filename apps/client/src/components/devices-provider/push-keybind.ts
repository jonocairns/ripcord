...
const isModifierToken = (token: string): token is keyof typeof MODIFIER_CODE_MAP => token in MODIFIER_CODE_MAP;

const serializePushKeybind = ({ code, ctrlKey, altKey, shiftKey, metaKey }: TKeybindState): string | undefined => {
	const normalizedCode = code.trim();

	if (!normalizedCode || normalizedCode === 'Unidentified' || isModifierCode(normalizedCode)) {
		return undefined;
	}
...
const matchesPushKeybind = (keyState: TKeybindState, keybind: string | undefined): boolean => {
...
const formatPushKeybindLabel = (keybind: string | undefined): string => {
	...
	return [
		...parts.map((modifier) => (modifier === 'Control' ? 'Ctrl' : modifier === 'Meta' ? 'Meta' : modifier)),
		formatKeyCode(code),
	].join(' + ');
};

export type { TKeybindState };
export { formatPushKeybindLabel, matchesPushKeybind, normalizePushKeybind, pushKeybindFromKeyState };
