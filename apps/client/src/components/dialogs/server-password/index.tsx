...
const ServerPasswordDialog = memo(({ isOpen, close, handshakeHash }: TServerPasswordDialogProps) => {
		...
		<AlertDialog open={isOpen}>
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						This server is password protected. Please enter the password to join.
					</AlertDialogDescription>
				</AlertDialogHeader>
				<div className="flex flex-col gap-2">
					<AutoFocus>
						<Input {...r('password')} className="mt-2" type="password" error={errors._general} />
					</AutoFocus>

					<p className="text-xs text-muted-foreground">Password will only be kept for this session.</p>
				</div>
				<AlertDialogFooter className="gap-2">
					<AlertDialogCancel onClick={onCancel}>Cancel</AlertDialogCancel>
					<AutoFocus>
						<AlertDialogAction onClick={onSubmit} disabled={!values.password || loading}>
							Join
						</AlertDialogAction>
					</AutoFocus>
				</AlertDialogFooter>
