...
const ChannelTypeItem = ({ icon, title, description, isActive, onClick }: TChannelTypeItemProps) => (
	<div
		className={cn(
...
const CreateChannelDialog = memo(
	({ isOpen, categoryId, close, defaultChannelType = ChannelType.TEXT }: TCreateChannelDialogProps) => {
			...
			<Dialog open={isOpen}>
				<DialogContent onInteractOutside={close} close={close}>
					...
					<DialogFooter className="gap-2">
						...
						<Button onClick={onSubmit} disabled={loading || !name || !channelType}>
							Create channel
						</Button>
					</DialogFooter>
				</DialogContent>
