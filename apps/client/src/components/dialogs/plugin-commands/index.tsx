...
const PluginCommandsDialog = memo(({ isOpen, close, pluginId }: TPluginCommandsDialogProps) => {
	...
	const handleArgChange = useCallback((argName: string, value: string, type: string) => {
		setCommandArgs((prev) => {
							...
							<>
								<div className="flex-1 overflow-y-auto p-6">
									<div className="max-w-2xl">
										<div className="mb-6">
											<h2 className="text-xl font-semibold mb-2">{selectedCommandInfo.name}</h2>
											{selectedCommandInfo.description && (
												<p className="text-sm text-muted-foreground">{selectedCommandInfo.description}</p>
											)}
										</div>

										{selectedCommandInfo.args && selectedCommandInfo.args.length > 0 ? (
											<Args
											...
											<div className="p-4 border rounded-lg bg-muted/30">
												<p className="text-sm text-muted-foreground">This command does not require any arguments.</p>
											</div>
										)}
								...
								<div className="border-t p-4 bg-muted/30">
									<div className="flex justify-start gap-4">
										...
										<Button onClick={handleExecute} disabled={!canExecute || isExecuting}>
