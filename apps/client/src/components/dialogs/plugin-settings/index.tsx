...
const SettingsList = memo(({ definitions, selectedKey, onSelect, dirtyKeys }: TSettingsListProps) => {
	return (
		<div className="w-80 border-r flex flex-col">
			...
			<div className="flex-1 overflow-y-auto">
				{definitions.length === 0 ? (
					<div className="p-4 text-sm text-muted-foreground text-center">No settings available for this plugin</div>
				) : (
					<div className="p-2">
						{definitions.map((def) => (
							<button
								...
								className={cn(
									...
									'hover:bg-muted',
									selectedKey === def.key && 'bg-primary text-primary-foreground hover:bg-primary/90',
								)}
							>
									...
									{dirtyKeys.has(def.key) && (
										<span
											className={cn(
												...
												selectedKey === def.key ? 'text-primary-foreground/80' : 'text-primary',
											)}
										>
											Edited
										</span>
								...
								<div
									className={cn(
										'text-xs mt-1',
										selectedKey === def.key ? 'text-primary-foreground/70' : 'text-muted-foreground',
									)}
								>
									{def.type}
								</div>
...
const PluginSettingsDialog = memo(({ isOpen, close, pluginId, pluginName }: TPluginSettingsDialogProps) => {
	...
	const dirtyKeys = useMemo(() => {
		...
		for (const def of definitions) {
			...
			if (def.type === 'number') {
				const parsed = currentValue === '' || currentValue === undefined ? NaN : Number(currentValue);
				...
				case 'boolean':
					return (
						<Switch
							...
							onCheckedChange={(checked) => handleDraftChange(def.key, checked)}
						/>
					);
				case 'number':
					return (
	...
	const handleSave = useCallback(async () => {
		...
		try {
			...
			for (const def of definitions) {
				...
				if (def.type === 'number') {
					const parsed = rawValue === '' || rawValue === undefined ? NaN : Number(rawValue);
								...
								<>
									<div className="flex-1 overflow-y-auto p-6">
										<div className="max-w-2xl">
											<div className="mb-6">
												<h2 className="text-xl font-semibold mb-2">{selectedSetting.name}</h2>
												{selectedSetting.description && (
													<p className="text-sm text-muted-foreground">{selectedSetting.description}</p>
												)}
												<div className="text-xs text-muted-foreground mt-2">Key: {selectedSetting.key}</div>
											</div>

											<div className="space-y-2">
												<Label htmlFor={`setting-${selectedSetting.key}`}>Value</Label>
