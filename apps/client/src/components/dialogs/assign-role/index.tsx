...
const AssignRoleDialog = memo(({ isOpen, close, user, refetch }: TAssignRoleDialogProps) => {
	...
	const availableRoles = useMemo(() => roles.filter((role) => !user.roleIds.includes(role.id)), [roles, user.roleIds]);
		...
		<AlertDialog open={isOpen}>
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					{isOwnUser && (
						<Alert variant="default">
							<Info />
							<AlertDescription>You are assigning a role to yourself.</AlertDescription>
						</Alert>
					)}
					{availableRoles.length === 0 && (
						<Alert variant="default">
							<Info />
							<AlertDescription>This user already has all available roles.</AlertDescription>
						</Alert>
					)}
				</AlertDialogHeader>
				<div className="flex flex-col gap-4">
					...
					{selectedRole && <PermissionsList permissions={selectedRole.permissions} variant="default" size="md" />}
				</div>
				<AlertDialogFooter className="gap-2">
					<AlertDialogCancel onClick={close}>Cancel</AlertDialogCancel>
					<AutoFocus>
						<AlertDialogAction onClick={onSubmit} disabled={availableRoles.length === 0 || selectedRoleId === 0}>
							Assign Role
						</AlertDialogAction>
					</AutoFocus>
				</AlertDialogFooter>
