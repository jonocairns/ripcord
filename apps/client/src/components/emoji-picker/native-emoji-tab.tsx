...
import { ALL_EMOJIS, EMOJI_CATEGORIES, type EmojiCategoryId, getEmojisByCategory, searchEmojis } from './emoji-data';
...
const CategoryBar = memo(({ activeCategory, onCategorySelect, hasRecentEmojis }: TCategoryBarProps) => (
			...
			return (
				<button
					...
					className={cn(
						...
						activeCategory === category.id ? 'bg-accent text-accent-foreground' : 'hover:bg-accent/50',
					)}
...
const NativeEmojiTab = memo(({ onEmojiSelect }: TNativeEmojiTabProps) => {
	...
	const effectiveCategory = activeCategory === 'recent' && !hasRecentEmojis ? 'people & body' : activeCategory;

	return (
		<div className="flex flex-col h-full">
			<div className="p-3 border-b mt-[-8px]">
				<Input placeholder="Search emojis..." value={search} onChange={handleSearchChange} className="h-9" autoFocus />
			</div>
			...
			<div className="flex-1 min-h-0">
				<EmojiGrid emojis={displayEmojis} onSelect={handleEmojiSelect} height={isSearching ? 260 : 220} />
			</div>
		</div>
	);
});
