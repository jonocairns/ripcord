...
const EmojiButton = memo(({ emoji, onSelect }: TEmojiButtonProps) => (
	...
	>
		{emoji.emoji ? (
			...
			<img src={emoji.fallbackImage} alt={emoji.name} className="w-6 h-6 object-contain" loading="lazy" />
		) : (
			<span className="text-xs text-muted-foreground truncate">{emoji.shortcodes[0]}</span>
		)}
	</button>
));
...
const EmojiGrid = memo(({ emojis, onSelect, height = 280 }: TEmojiGridProps) => {
	const rowCount = useMemo(() => Math.ceil(emojis.length / GRID_COLS), [emojis.length]);
	...
	if (emojis.length === 0) {
		return <div className="flex items-center justify-center h-32 text-muted-foreground text-sm">No emojis found</div>;
	}
