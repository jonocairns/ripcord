...
const loadRecentEmojis = (): TEmojiItem[] => {
	...
	const stored = getLocalStorageItemAsJSON<StoredEmoji[]>(LocalStorageKey.RECENT_EMOJIS, []);
...
const subscribe = (callback: () => void): (() => void) => {
	...
	return () => {
		window.removeEventListener(RECENT_EMOJIS_CHANGED_EVENT, handleRecentEmojisChanged);
