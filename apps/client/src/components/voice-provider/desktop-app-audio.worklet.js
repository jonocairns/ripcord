class PcmQueueProcessor extends AudioWorkletProcessor {
	constructor(options) {
		...
		this.targetChunks = Math.max(1, options.processorOptions?.targetChunks || 4);
	...
	process(_inputs, outputs) {
		...
		while (writtenFrames < frameCount) {
			...
			const availableFrames = this.currentChunk.length / this.channels - this.chunkFrameOffset;
			...
			for (let frameOffset = 0; frameOffset < framesToCopy; frameOffset += 1) {
				...
				for (let channelIndex = 0; channelIndex < output.length; channelIndex += 1) {
					...
					output[channelIndex][outputFrameIndex] = this.currentChunk[sourceBaseOffset + sourceChannelIndex] || 0;
				}
			}
		...
		if (this.emitQueueTelemetry && currentTime - this.lastTelemetryAt >= this.telemetryIntervalSeconds) {
			let queuedFrames = 0;
			...
			if (this.currentChunk) {
				queuedFrames += this.currentChunk.length / this.channels - this.chunkFrameOffset;
			}
