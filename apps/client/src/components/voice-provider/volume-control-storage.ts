...
const loadVolumesFromStorage = (): TVolumeSettings => {
	try {
		return getLocalStorageItemAsJSON<TVolumeSettings>(LocalStorageKey.VOLUME_SETTINGS) ?? {};
	} catch {
		return {};
	}
};
...
const dispatchVolumeSettingsUpdated = (detail: TVolumeSettingsUpdatedDetail) => {
	...
	window.dispatchEvent(new CustomEvent<TVolumeSettingsUpdatedDetail>(VOLUME_SETTINGS_UPDATED_EVENT, { detail }));
};
