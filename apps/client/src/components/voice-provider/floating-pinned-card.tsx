...
const FloatingPinnedCard = memo(
	({ remoteUserStreams, externalStreams, localVideoStream, localScreenShareStream }: TFloatingPinnedCardProps) => {
		...
		const isExternalStream = pinnedCard?.type === PinnedCardType.EXTERNAL_STREAM;

		const floatingCardContent = useMemo<TFloatingCardContent | undefined>(() => {
			if (pinnedCard) {
				if (isExternalStream) {
		...
		return (
			...
			>
				<CardControls>
					...
					/>
					<IconButton icon={SendToBack} size="sm" variant="ghost" title="Reset Position" onClick={resetCard} />
					<IconButton icon={X} size="sm" variant="ghost" title="Close" onClick={onCloseClick} />
				</CardControls>
