class MicCaptureProcessor extends AudioWorkletProcessor {
	constructor(options) {
		...
		this.targetFrameSize = Math.max(1, options.processorOptions?.targetFrameSize || 480);
		this.pendingChunks = [];
	...
	process(inputs, outputs) {
		...
		for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
			for (let channelIndex = 0; channelIndex < this.channels; channelIndex += 1) {
				...
				interleaved[frameIndex * this.channels + channelIndex] = input[sourceChannelIndex]?.[frameIndex] || 0;
			}
		}
