...
const createNcDiagnosticsAggregator = (sessionId: string) => {
	...
	const push = (frame: TVoiceFilterPcmFrame) => {
		...
		if (d.aecDoubleTalkConfidence !== undefined && d.aecDoubleTalkConfidence >= 0.6) {
			aecDoubleTalkFrames++;
		}
	...
	const logSnapshot = () => {
		...
		const lsnrRange =
			snapshot.lsnrMin !== null && snapshot.lsnrMax !== null
				...
				: 'n/a';
		const agc = snapshot.agcGainMean !== null ? `${snapshot.agcGainMean.toFixed(2)}×` : 'off';
		const aecErle = snapshot.aecErleMean !== null ? `${snapshot.aecErleMean.toFixed(1)} dB` : 'off';
		const aecDelay = snapshot.aecDelayMs !== null ? `${snapshot.aecDelayMs.toFixed(0)} ms` : 'n/a';
...
const createNativeDesktopMicAudioProcessingPipeline = async ({
	...
	const session = await desktopBridge.startVoiceFilterSession({
		...
		dereverbMode: (noiseSuppression ? 'tail' : 'off') as TRuntimeVoiceDereverbMode,
	});
	...
	const firstFilteredFramePromise = new Promise<void>((resolve, reject) => {
		...
		const timeout = window.setTimeout(() => {
			...
			settled = true;
			reject(new Error(`Native voice filter produced no frames (session=${session.sessionId})`));
	...
	const removeFrameSubscription = desktopBridge.subscribeVoiceFilterFrames((frame: TVoiceFilterPcmFrame) => {
	...
	const removeStatusSubscription = desktopBridge.subscribeVoiceFilterStatus((statusEvent: TVoiceFilterStatusEvent) => {
		...
		if (statusEvent.reason !== 'capture_stopped') {
			...
			if (statusEvent.error) {
				console.warn('[voice-filter-debug] Native voice filter status error detail', statusEvent.error);
			}
		}

		if (!hasReceivedFilteredFrame) {
			settleFirstFilteredFrame?.(new Error(`Native voice filter ended before frames (${statusEvent.reason})`));
		}
	});
	...
	if (typeof AudioWorkletNode === 'undefined' || typeof captureContext.audioWorklet === 'undefined') {
		await outputPipeline.destroy();
		await desktopBridge.stopVoiceFilterSession(session.sessionId);
	...
	workletNode.port.onmessage = (messageEvent) => {
		...
		const frameCount =
			typeof data.frameCount === 'number' ? Math.floor(data.frameCount) : Math.floor(samples.length / session.channels);
...
const createNativeSidecarMicCapturePipeline = async ({
	...
	const removeStatusSubscription = desktopBridge.subscribeVoiceFilterStatus((statusEvent: TVoiceFilterStatusEvent) => {
		...
		if (!hasReceivedFilteredFrame) {
			settleFirstFilteredFrame?.(new Error(`Native sidecar mic-capture ended before frames (${statusEvent.reason})`));
		}
	});
...
const resolveSidecarDeviceId = async (
	...
	try {
		...
		return sidecarResult.devices.find((d) => d.label.trim().toLowerCase() === browserLabel)?.id;
	} catch {
		return undefined;
	}
};
