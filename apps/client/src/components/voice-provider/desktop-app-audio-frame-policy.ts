...
const MAX_RECOVERABLE_MISSING_FRAMES = Math.max(MAX_RECOVERABLE_DROPPED_FRAMES, MAX_RECOVERABLE_SEQUENCE_GAP_FRAMES);
...
const computeRecoverableMissingFrameCount = (missingFrameCount: number): number => {
	...
	return Math.min(Math.max(0, Math.floor(missingFrameCount)), MAX_RECOVERABLE_MISSING_FRAMES);
};
...
const validateDesktopAppAudioFrame = ({
	...
	if (!Number.isInteger(frame.sampleRate) || frame.sampleRate <= 0 || frame.sampleRate !== sessionSampleRate) {
		return {
			accepted: false,
...
export { computeRecoverableMissingFrameCount, isPcmFrame, validateDesktopAppAudioFrame };
