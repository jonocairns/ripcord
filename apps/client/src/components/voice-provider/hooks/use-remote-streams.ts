...
const useRemoteStreams = () => {
	...
	const addExternalStreamTrack = useCallback(
		(streamId: number, stream: MediaStream, kind: StreamKind.EXTERNAL_AUDIO | StreamKind.EXTERNAL_VIDEO) => {
	...
	const removeExternalStreamTrack = useCallback(
		(streamId: number, kind: StreamKind.EXTERNAL_AUDIO | StreamKind.EXTERNAL_VIDEO) => {
			setExternalStreams((prev) => {
				const existing = prev[streamId];
				...
				} else if (kind === StreamKind.EXTERNAL_VIDEO && streamEntry.videoStream) {
	...
	const addRemoteUserStream = useCallback((userId: number, stream: MediaStream, kind: TRemoteUserStreamKinds) => {
	...
	const removeRemoteUserStream = useCallback((userId: number, kind: TRemoteUserStreamKinds) => {
		setRemoteUserStreams((prev) => {
