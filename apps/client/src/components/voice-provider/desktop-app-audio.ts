...
const createDesktopAppAudioPipeline = async (
	...
	return {
		...
		pushFrame: (frame) => {
			...
			if (!frameValidation.accepted) {
				switch (frameValidation.reason) {
					...
					case 'unsupported-protocol':
						console.warn(`[${logLabel}] Unsupported app audio protocol version`, frame.protocolVersion);
						return;
					...
					case 'sample-rate-mismatch': {
						...
						if (now >= nextMalformedFrameLogAt) {
							console.warn(`[${logLabel}] Dropping app audio frame with sample-rate mismatch`, {
			...
			const { droppedFrameCount, missingFrameCount, sequenceGapFrames } = frameValidation;
