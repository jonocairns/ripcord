...
const FullScreenImage = memo((props: TFullScreenImageProps) => {
	...
	const portalContainer = createPortal(
		<>
			...
			>
				...
				/>
				<Button onClick={onCloseClick} size="icon" variant="outline" className="absolute top-2 right-2 z-50">
					<X size="1.1rem" />
				</Button>
			</div>
		</>,
	...
	return (
		<>
			<img {...props} className={cn('cursor-pointer', props.className)} onClick={onOpenClick} draggable={false} />
			{portalContainer}
		</>
	);
});
