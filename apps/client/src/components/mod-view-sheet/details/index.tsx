...
const Row = memo(({ icon, label, value, details }: TRowProps) => {
	let valContent = <span className="text-sm text-muted-foreground truncate max-w-[160px]">{value}</span>;
		...
		<Card>
			...
			<CardContent className="space-y-2">
				<div className="space-y-2">
					...
					{user.banned && (
						<>
							<Row icon={<Gavel className="h-4 w-4 text-muted-foreground" />} label="Banned" value="Yes" />
