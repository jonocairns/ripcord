...
const TopBar = memo(({ onToggleRightSidebar, isOpen, onToggleVoiceChat, isVoiceChatOpen }: TTopBarProps) => {
		...
		<div className="hidden lg:flex h-8 w-full bg-card border-b border-border items-center justify-end px-4 transition-all duration-300 ease-in-out gap-2">
			{isCurrentVoiceChannelSelected && currentVoiceChannelId && (
				<>
					...
					>
						<Tooltip content={isVoiceChatOpen ? 'Close Voice Chat' : 'Open Voice Chat'} asChild={false}>
							<MessageSquare
								className={cn('w-4 h-4 transition-all duration-200 ease-in-out', isVoiceChatOpen && 'fill-current')}
							/>
						</Tooltip>
					</Button>
				</>
