...
const AudioStreamControl = memo(({ userId, volumeKey, type, name }: AudioStreamControlProps) => {
	...
	return (
		<div className="flex items-center gap-3 py-2">
			<div className="flex items-center gap-2 flex-1 min-w-0">
				...
				{type === AudioStreamType.ScreenShare && <Monitor className="h-3 w-3 text-muted-foreground" />}
			</div>

			<div className="flex items-center gap-2 flex-shrink-0">
				<Button variant="ghost" size="sm" onClick={() => toggleMute(volumeKey)} className="h-6 w-6 p-0">
...
const VolumeController = memo(({ channelId }: VolumeControllerProps) => {
	...
	const audioStreams = useMemo(() => {
		...
		return streams;
	}, [voiceUsers, externalAudioStreams, ownUserId, getUserVolumeKey, getUserScreenVolumeKey, getExternalVolumeKey]);

	return (
		<Popover>
			<PopoverTrigger asChild>
				<Button variant="ghost" size="sm" className="h-6 px-2 transition-all duration-200 ease-in-out">
				...
				<div className="space-y-2">
					<div className="flex items-center justify-between mb-3">
						...
						<span className="text-xs text-muted-foreground">
							{audioStreams.length} {audioStreams.length === 1 ? 'stream' : 'streams'}
						</span>
					</div>
					...
					<div className="space-y-1 max-h-96 overflow-y-auto">
						...
						{audioStreams.length === 0 && (
							<div className="text-sm text-muted-foreground py-4 text-center">No remote audio streams available.</div>
						)}
					</div>
				</div>
			</PopoverContent>
