				...
				<div className="divide-y">
					{paginatedItems.length === 0 ? (
						<div className="px-4 py-8 text-center text-muted-foreground text-sm">
							{searchTerm.trim() ? `No items found matching "${searchTerm}"` : emptyMessage}
						</div>
					) : (
						paginatedItems.map((item, index) => (
							<div key={index}>{renderRow(item, (currentPage - 1) * itemsPerPage + index)}</div>
						))
					)}
				</div>
			</div>
				...
				<div className="text-xs text-muted-foreground">
					{filteredItems.length === 0 ? (
						...
						<>
							...
							{Math.min(currentPage * itemsPerPage, filteredItems.length)} of {filteredItems.length} item
...
const PaginatedTable = memo(PaginatedTableComponent) as <T>(props: TPaginatedTableProps<T>) => React.ReactElement;
