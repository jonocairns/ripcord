...
const ChannelSettings = memo(({ close, channelId }: TChannelSettingsProps) => {
	...
	const [activeSection, setActiveSection] = useState<TChannelSettingsValue>('general');
			...
			<div className="mx-auto flex w-full max-w-7xl gap-6 h-full">
				<aside className="w-full max-w-xs shrink-0 py-6">
					<div className="space-y-4">
						<div className="px-2">
							<p className="truncate text-xl font-semibold">#{channel?.name ?? 'channel'}</p>
							<p className="text-sm text-muted-foreground">Manage channel settings</p>
						</div>
						...
						<div className="max-h-[calc(100vh-15rem)] overflow-y-auto px-2 pb-2">
							...
							<nav className="space-y-1">
								{filteredSections.map((section) => {
									const Icon =
										section.value === 'general' ? SlidersHorizontal : section.value === 'permissions' ? Shield : Lock;
				...
				<section className="min-w-0 flex-1 py-6">
					{selectedSection.value === 'general' && <General channelId={channelId} />}
					{selectedSection.value === 'permissions' && <ChannelPermissions channelId={channelId} />}
					{selectedSection.value === 'security' && <Security channelId={channelId} />}
				</section>
			</div>
		</ServerScreenLayout>
	);
