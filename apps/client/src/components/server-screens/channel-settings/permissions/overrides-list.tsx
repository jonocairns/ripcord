import type { TChannelRolePermission, TChannelUserPermission } from '@sharkord/shared';
...
const RoleItem = memo(({ roleId, selectedKey, setSelectedKey }: TRoleItemProps) => {
...
const UserItem = memo(({ userId, selectedKey, setSelectedKey }: TUserItemProps) => {
		...
		>
			<div className="flex items-center gap-2">
				<Avatar className="h-5 w-5">
					<AvatarFallback className={cn('text-[10px] text-white', getAvatarColor(userId))}>
...
const RolesSection = memo(({ rolePermissions, selectedKey, setSelectedKey }: TRolesSectionProps) => {
	const roleIds = useMemo(() => Array.from(new Set(rolePermissions.map((perm) => perm.roleId))), [rolePermissions]);

	if (roleIds.length === 0) return null;

	return (
		<>
			<div className="px-3 py-1 text-xs font-semibold uppercase text-muted-foreground">Roles</div>
			{roleIds.map((roleId) => (
				<RoleItem key={roleId} roleId={roleId} selectedKey={selectedKey} setSelectedKey={setSelectedKey} />
			))}
		</>
	);
});
...
const UsersSection = memo(({ userPermissions, selectedKey, setSelectedKey }: TUsersSectionProps) => {
	const userIds = useMemo(() => Array.from(new Set(userPermissions.map((perm) => perm.userId))), [userPermissions]);

	if (userIds.length === 0) return null;

	return (
		<>
			<div className="px-3 py-1 text-xs font-semibold uppercase text-muted-foreground">Users</div>
			{userIds.map((userId) => (
				<UserItem key={userId} userId={userId} selectedKey={selectedKey} setSelectedKey={setSelectedKey} />
			))}
		</>
	);
});
			...
			<Card className="w-64 flex-shrink-0">
				<CardHeader>
					<div className="flex items-center justify-between">
						...
						<SearchPopover onSelect={onSelect} ignoreRoleIds={usedRolesIds} ignoreUserIds={usedUserIds} />
					</div>
				</CardHeader>
				<CardContent className="space-y-2 p-2">
					{isEmpty ? (
						<div className="py-8 text-center text-sm text-muted-foreground">No permission overrides yet</div>
					) : (
						<>
							<RolesSection
