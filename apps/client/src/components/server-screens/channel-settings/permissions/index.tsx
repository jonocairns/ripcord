...
import { useAdminChannelGeneral, useAdminChannelPermissions } from '@/features/server/admin/hooks';
...
const ChannelPermissions = memo(({ channelId }: TChannelPermissionsProps) => {
	const [selectedOverrideId, setSelectedOverrideId] = useState<string | undefined>();
	const { channel } = useAdminChannelGeneral(channelId);
	const { rolePermissions, userPermissions, loading, refetch } = useAdminChannelPermissions(channelId);
		...
		<Card>
			<CardHeader>
				...
				<CardDescription className="flex flex-col space-y-4">
					<span>
						Manage channel-specific permissions for roles and users. These permissions are not inherited from
						server-level permissions. User permissions take precedence over role permissions.
					</span>
					{!channel?.private && (
						<Alert variant="destructive">
							...
							<AlertDescription>
								This is a public channel; everyone has access by default. These permissions will not be applied unless
								the channel is set to private. You can do this in the General Settings tab.
							</AlertDescription>
						</Alert>
					)}
				</CardDescription>
