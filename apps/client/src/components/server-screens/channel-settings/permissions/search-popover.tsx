...
const SearchPopover = memo(({ onSelect, ignoreUserIds, ignoreRoleIds }: TSearchPopoverProps) => {
	...
	const filteredRoles = useMemo(
		() =>
			roles.filter(
				(role) => role.name.toLowerCase().includes(searchQuery.toLowerCase()) && !ignoreRoleIds?.includes(role.id),
			),
	...
	const filteredUsers = useMemo(
		() =>
			users.filter(
				(user) => user.name.toLowerCase().includes(searchQuery.toLowerCase()) && !ignoreUserIds?.includes(user.id),
			),
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent className="w-80 p-0" align="end">
				<Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TChannelPermissionType)}>
