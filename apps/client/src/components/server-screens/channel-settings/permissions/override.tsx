...
const Override = memo(({ channelId, overrideId, permissions, setSelectedOverrideId, refetch }: TOverrideProps) => {
	const [localPermissions, setLocalPermissions] = useState<TChannelPermission[]>(permissions);
	...
	const onDeleteOverride = useCallback(async () => {
		...
		try {
			...
			toast.error(getTrpcError(error, 'Failed to delete permission override'));
		}
	...
	const onUpdateOverride = useCallback(async () => {
		...
		try {
			...
			const allowedPermissions = localPermissions.filter((perm) => perm.allow).map((perm) => perm.permission);
			...
			toast.error(getTrpcError(error, 'Failed to update permission override'));
		}
	...
	const onTogglePermission = useCallback((permission: ChannelPermission) => {
		setLocalPermissions((prevPermissions) =>
			prevPermissions.map((perm) => (perm.permission === permission ? { ...perm, allow: !perm.allow } : perm)),
		);
	}, []);
		...
		<Card className="flex-1">
			<CardHeader>
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-3">
						{overrideType === 'role' ? <RoleHeader roleId={targetId} /> : <UserHeader userId={targetId} />}
					</div>
					<Button
						size="icon"
						variant="ghost"
			...
			<CardContent className="space-y-6">
				<ChannelPermissionList permissions={localPermissions} onTogglePermission={onTogglePermission} />

				<div className="flex justify-end gap-2 pt-4">
					<Button variant="outline" onClick={() => setSelectedOverrideId(undefined)}>
						Cancel
					</Button>
