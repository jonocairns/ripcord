import { Search, SlidersHorizontal } from 'lucide-react';
import { memo, useMemo, useState } from 'react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useCategoryById } from '@/features/server/categories/hooks';
import { cn } from '@/lib/utils';
import type { TServerScreenBaseProps } from '../screens';
import { ServerScreenLayout } from '../server-screen-layout';
import { General } from './general';

type TCategorySettingsProps = TServerScreenBaseProps & {
	categoryId: number;
};
type TCategorySettingsValue = 'general';
type TCategorySettingsSection = {
	keywords: string[];
	label: string;
	value: TCategorySettingsValue;
};

const CATEGORY_SETTINGS_SECTIONS: TCategorySettingsSection[] = [
	{
		value: 'general',
		label: 'General',
		keywords: ['general', 'name', 'category'],
	},
];

const CategorySettings = memo(({ close, categoryId }: TCategorySettingsProps) => {
	const category = useCategoryById(categoryId);
	const [activeSection, setActiveSection] = useState<TCategorySettingsValue>('general');
	const [search, setSearch] = useState('');

	const filteredSections = useMemo(() => {
		const normalizedSearch = search.trim().toLowerCase();

		if (!normalizedSearch) {
			return CATEGORY_SETTINGS_SECTIONS;
		}

		return CATEGORY_SETTINGS_SECTIONS.filter(({ label, keywords }) => {
			return (
				label.toLowerCase().includes(normalizedSearch) || keywords.some((keyword) => keyword.includes(normalizedSearch))
			);
		});
	}, [search]);

	const selectedSection = useMemo(() => {
		return (
			filteredSections.find(({ value }) => value === activeSection) ??
			filteredSections[0] ??
			CATEGORY_SETTINGS_SECTIONS[0]
		);
	}, [activeSection, filteredSections]);

	return (
		<ServerScreenLayout close={close} title="Category Settings">
			<div className="mx-auto flex h-full w-full max-w-7xl gap-6">
				<aside className="w-full max-w-xs shrink-0 py-6">
					<div className="space-y-4">
						<div className="px-2">
							<p className="truncate text-xl font-semibold">{category?.name ?? 'category'}</p>
							<p className="text-sm text-muted-foreground">Manage category settings</p>
						</div>

						<div className="relative px-2">
							<Search className="pointer-events-none absolute top-1/2 left-5 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
							<Input
								value={search}
								onChange={(event) => setSearch(event.target.value)}
								placeholder="Search"
								className="pl-9"
							/>
						</div>

						<div className="max-h-[calc(100vh-15rem)] overflow-y-auto px-2 pb-2">
							<p className="mb-2 text-xs font-semibold tracking-wide text-muted-foreground uppercase">
								Category Settings
							</p>
							<nav className="space-y-1">
								{filteredSections.map((section) => {
									return (
										<Button
											key={section.value}
											variant="ghost"
											onClick={() => setActiveSection(section.value)}
											className={cn(
												'w-full justify-start gap-3 px-3',
												selectedSection.value === section.value
													? 'bg-muted text-foreground hover:bg-muted'
													: 'text-muted-foreground hover:text-foreground',
											)}
										>
											<SlidersHorizontal className="h-4 w-4" />
											{section.label}
										</Button>
									);
								})}
							</nav>

							{filteredSections.length === 0 && (
								<p className="px-3 py-2 text-sm text-muted-foreground">No settings found.</p>
							)}
						</div>
					</div>
				</aside>

				<section className="min-w-0 flex-1 py-6">
					{selectedSection.value === 'general' && <General categoryId={categoryId} />}
				</section>
			</div>
		</ServerScreenLayout>
	);
});

export { CategorySettings };
