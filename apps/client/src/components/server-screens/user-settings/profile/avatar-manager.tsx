...
const AvatarManager = memo(({ user }: TAvatarManagerProps) => {
	...
	const removeAvatar = useCallback(async () => {
		...
		try {
			...
			toast.error(getTrpcError(error, 'Could not remove avatar. Please try again.'));
		}
	}, []);
	...
	const onAvatarClick = useCallback(async () => {
		...
		try {
			...
			toast.error(getTrpcError(error, 'Could not update avatar. Please try again.'));
		}
	}, [openFilePicker]);
	...
	return (
		<div className="space-y-2">
			<div className="space-y-1">
				...
				<p className="text-xs text-muted-foreground">Upload a new avatar image.</p>
			</div>
