...
const UserSettings = memo(({ close, initialSection }: TUserSettingsProps) => {
	...
	const [activeSection, setActiveSection] = useState<TUserSettingsSectionValue>(initialSection || 'profile');
	...
	const selectedSection = useMemo(() => {
		return (
			filteredSections.find(({ value }) => value === activeSection) ?? filteredSections[0] ?? USER_SETTINGS_SECTIONS[0]
		);
				...
				<aside className="w-full max-w-xs shrink-0 py-6">
					<div className="space-y-4">
						<div className="flex items-center gap-3 px-2">
							{ownPublicUser && <UserAvatar userId={ownPublicUser.id} showUserPopover={false} className="h-12 w-12" />}
							<div className="min-w-0">
								<p className="truncate text-xl font-semibold">{ownPublicUser?.name ?? 'User'}</p>
						...
						<div className="max-h-[calc(100vh-15rem)] overflow-y-auto px-2 pb-2">
							<p className="mb-2 text-xs font-semibold tracking-wide text-muted-foreground uppercase">Settings</p>
							<nav className="space-y-1">
								{filteredSections.map((section) => {
									const Icon =
										section.value === 'profile' ? UserRound : section.value === 'devices' ? Laptop2 : KeyRound;
