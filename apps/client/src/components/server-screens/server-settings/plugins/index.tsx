...
import { AlertCircle, FileText, Package, RefreshCw, Settings, Terminal, User } from 'lucide-react';
		...
		<div className="flex items-start gap-4 p-4 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
			<div className="flex-shrink-0">
				{plugin.logo ? (
					...
					<div className={cn('w-12 h-12 rounded-md bg-muted flex items-center justify-center')}>
			...
			<div className="flex-1 min-w-0 space-y-2">
				<div className="flex items-start justify-between gap-3">
					<div className="flex-1 min-w-0">
						<h3 className="font-semibold text-base leading-tight">{plugin.name}</h3>
						<p className="text-sm text-muted-foreground mt-1 line-clamp-2">{plugin.description}</p>
					</div>
					<div className="flex items-center gap-2 shrink-0">
						<Button variant="ghost" size="sm" onClick={handleViewLogs} className="h-8">
							<FileText className="w-4 h-4 mr-1.5" />
							Logs
						</Button>
						<Button variant="ghost" size="sm" onClick={handleViewCommands} className="h-8" disabled={!plugin.enabled}>
							<Terminal className="w-4 h-4 mr-1.5" />
							Commands
						</Button>
						<Button variant="ghost" size="sm" onClick={handleViewSettings} className="h-8" disabled={!plugin.enabled}>
				...
				{plugin.loadError && (
					<Alert variant="destructive" className="py-2">
						...
						<AlertDescription className="text-xs">{plugin.loadError}</AlertDescription>
					</Alert>
				)}
			...
			<CardHeader>
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription>
							Manage installed plugins and extend Ripcord with additional features and functionality.
						</CardDescription>
					</div>
					<Button
						variant="outline"
			...
			<CardContent>
				{enabled ? (
						...
						{plugins.length === 0 ? (
							<div className="flex flex-col items-center justify-center py-12 text-center">
								...
								<p className="text-sm text-muted-foreground max-w-sm">
									Install plugins to add new features and extend the functionality of Ripcord.
								</p>
							</div>
						) : (
					...
					<div className="flex flex-col items-center justify-center py-12 text-center">
						...
						<p className="text-sm text-muted-foreground max-w-sm">
							Plugins have been disabled for this server. Enable plugins in the server settings to manage and use
							plugins.
						</p>
					</div>
				)}
