...
const Storage = memo(() => {
	const { values, loading, submit, onChange, labels, diskMetrics } = useAdminStorage();
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Manage your server's storage settings. Control how data is stored, accessed, and managed. Here you can
					configure storage limits, backup options, and data retention policies to ensure optimal performance and
					reliability.
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				...
				>
					<Switch
						...
						onCheckedChange={(checked) => onChange('storageUploadEnabled', checked)}
					/>
				</Group>
				...
				>
					<Slider
						...
						step={FILE_SIZE_STEP}
						disabled={!values.storageUploadEnabled}
						onValueChange={(values) => onChange('storageUploadMaxFileSize', values[0])}
						rightSlot={
							<span className="text-sm">
								{labels.storageUploadMaxFileSize.value} {labels.storageUploadMaxFileSize.unit}
							</span>
						}
					/>
				</Group>
				...
				>
					<Slider
						...
						step={FILE_SIZE_STEP}
						disabled={!values.storageUploadEnabled}
						onValueChange={(values) => onChange('storageSpaceQuotaByUser', values[0])}
						rightSlot={
							<span className="text-sm">
								{labels.storageSpaceQuotaByUser.value} {labels.storageSpaceQuotaByUser.unit}
							</span>
						}
					/>
				</Group>

				<Group label="Overflow action" description="Action to take when the global storage quota is exceeded.">
					<Select
						onValueChange={(value) => onChange('storageOverflowAction', value as StorageOverflowAction)}
