...
type TServerSettingsValue = 'emojis' | 'general' | 'invites' | 'plugins' | 'roles' | 'storage' | 'updates' | 'users';
...
const ServerSettings = memo(({ close }: TServerSettingsProps) => {
	...
	const [activeSection, setActiveSection] = useState<TServerSettingsValue>(defaultSection);
	...
	const filteredSections = useMemo(() => {
		...
		return SERVER_SETTINGS_SECTIONS.filter(({ label, keywords }) => {
			return (
				label.toLowerCase().includes(normalizedSearch) || keywords.some((keyword) => keyword.includes(normalizedSearch))
			);
		});
	}, [search]);

	const selectedSection = useMemo(() => {
		const allowedSections = SERVER_SETTINGS_SECTIONS.filter((section) => can(section.permission));
		const allowedFilteredSections = filteredSections.filter((section) => can(section.permission));
			...
			<div className="mx-auto flex w-full max-w-7xl gap-6 h-full">
				<aside className="w-full max-w-xs shrink-0 py-6">
					<div className="space-y-4">
						<div className="px-2">
							<p className="truncate text-xl font-semibold">{serverName ?? 'Server'}</p>
				...
				<section className="min-w-0 flex-1 py-6">
					{selectedSection?.value === 'general' && can(Permission.MANAGE_SETTINGS) && <General />}
					{selectedSection?.value === 'roles' && can(Permission.MANAGE_ROLES) && <Roles />}
					{selectedSection?.value === 'emojis' && can(Permission.MANAGE_EMOJIS) && <Emojis />}
					{selectedSection?.value === 'storage' && can(Permission.MANAGE_STORAGE) && <Storage />}
					{selectedSection?.value === 'users' && can(Permission.MANAGE_USERS) && <Users />}
					{selectedSection?.value === 'invites' && can(Permission.MANAGE_INVITES) && <Invites />}
					{selectedSection?.value === 'updates' && can(Permission.MANAGE_UPDATES) && <Updates />}
					{selectedSection?.value === 'plugins' && can(Permission.MANAGE_PLUGINS) && <Plugins />}

					{!selectedSection && (
						<div className="rounded-md border border-border bg-card p-6">
							<p className="text-sm text-muted-foreground">You do not have permission to view server settings.</p>
						</div>
					)}
				</section>
			</div>
