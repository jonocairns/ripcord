...
const UpdateEmoji = memo(({ selectedEmoji, setSelectedEmojiId, refetch }: TUpdateEmojiProps) => {
		...
		<Card className="flex-1 gap-0 py-0">
			<CardHeader className="border-b py-6">
				<div className="flex items-center justify-between">
					<div className="space-y-1">
						...
						<CardDescription>Update the emoji name or remove it from the server.</CardDescription>
					</div>
					<Button size="icon" variant="ghost" onClick={onDeleteEmoji} title="Delete emoji">
			...
			<CardContent className="flex-1 space-y-6 overflow-y-auto py-6">
				<div className="bg-muted flex items-center gap-4 rounded-lg border p-4">
					<Emoji src={getFileUrl(selectedEmoji.file)} name={selectedEmoji.name} className="h-16 w-16" />
					<div className="min-w-0">
						<div className="font-medium">:{selectedEmoji.name}:</div>
						<div className="text-sm text-muted-foreground">
							{filesize(selectedEmoji.file.size)} • Uploaded by {selectedEmoji.user.name}
						</div>
					</div>
				</div>
				...
				<div className="space-y-4">
					<div className="space-y-2">
						...
						<p className="text-xs text-muted-foreground">
							This emoji will be used as :{name || selectedEmoji.name}: in messages
						</p>
					</div>
				</div>
			</CardContent>
			<CardFooter className="justify-end gap-2 py-4">
				<Button variant="outline" onClick={() => setSelectedEmojiId(undefined)}>
					Close
				</Button>
				<Button onClick={onUpdateEmoji} disabled={selectedEmoji.name === name}>
					Apply
				</Button>
			</CardFooter>
		</Card>
