...
const InvitesTable = memo(({ invites, refetch }: TInvitesTableProps) => {
	const searchFilter = useCallback((invite: TJoinedInvite, searchTerm: string) => {
		const query = searchTerm.toLowerCase();

		return invite.code.toLowerCase().includes(query) || invite.creator.name.toLowerCase().includes(query);
	}, []);

	return (
		<PaginatedTable
			items={invites}
			renderRow={(invite) => <TableInvite key={invite.id} invite={invite} refetch={refetch} />}
