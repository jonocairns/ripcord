...
const Disconnected = memo(({ info }: TDisconnectedProps) => {
	const hasDesktopBridge = typeof window !== 'undefined' && Boolean(window.sharkordDesktop);
			...
			try {
				...
				if (isNonRetriableTrpcError(error)) {
					setDisconnectInfo({
						...
						reason: 'Reconnect stopped because the server rejected the request (4xx).',
						wasClean: false,
						time: new Date(),
					});

					if (opts.manual) {
						toast.error('Reconnect stopped: server rejected the request (4xx).');
					}
		...
		<div className="flex items-center justify-center min-h-screen bg-background">
			<div className="text-center space-y-6 max-w-md px-6">
				...
				<div className="space-y-2">
					<h1 className="text-2xl font-semibold text-foreground">{disconnectType.title}</h1>
					<p className="text-muted-foreground text-sm leading-relaxed">{disconnectType.message}</p>
					{disconnectType.autoReconnect && hasSavedAuth && (
						<p className="text-muted-foreground text-xs leading-relaxed">Trying to reconnect every 5 seconds.</p>
					)}
				</div>

				{disconnectType.canReconnect && (
					<Button onClick={handleReconnect} className="inline-flex items-center gap-2" disabled={reconnecting}>
						<RefreshCw className={reconnecting ? 'h-4 w-4 animate-spin' : 'h-4 w-4'} />
