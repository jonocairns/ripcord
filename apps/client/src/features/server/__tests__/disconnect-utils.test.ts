...
import {
  isReconnectPausedDisconnectCode,
  shouldRestoreVoiceAfterDisconnect
} from '../disconnect-utils';

describe('shouldRestoreVoiceAfterDisconnect', () => {
  it('keeps the previous voice channel for unexpected transport closes', () => {
    expect(shouldRestoreVoiceAfterDisconnect(DisconnectCode.UNEXPECTED)).toBe(
      true
    );
  ...
  it('does not keep the previous voice channel for forced removals', () => {
    expect(shouldRestoreVoiceAfterDisconnect(DisconnectCode.KICKED)).toBe(
      false
    );
    expect(shouldRestoreVoiceAfterDisconnect(DisconnectCode.BANNED)).toBe(
      false
    );
  });
});
...
describe('isReconnectPausedDisconnectCode', () => {
  ...
  it('allows reconnect for server shutdowns and generic transport drops', () => {
    expect(
      isReconnectPausedDisconnectCode(DisconnectCode.SERVER_SHUTDOWN)
    ).toBe(false);
    expect(isReconnectPausedDisconnectCode(DisconnectCode.UNEXPECTED)).toBe(
      false
    );
  });
});
