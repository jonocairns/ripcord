...
import { addMessages, addTypingUser, deleteMessage, updateMessage } from './actions';

const subscribeToMessages = () => {
	const trpc = getTRPCClient();

	const onMessageSub = trpc.messages.onNew.subscribe(undefined, {
		onData: (message: TJoinedMessage) => addMessages(message.channelId, [message], {}, true),
	...
	const onMessageUpdateSub = trpc.messages.onUpdate.subscribe(undefined, {
		onData: (message: TJoinedMessage) => updateMessage(message.channelId, message),
