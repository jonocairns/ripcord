import { ChannelPermission, OWNER_ROLE_ID, type Permission } from '@sharkord/shared';
...
export const usePublicServerSettings = () => useServerStore(publicServerSettingsSelector);
...
export const useIsOwnUserOwner = () => {
	...
	return useMemo(() => ownUserRoles.some((role) => role.id === OWNER_ROLE_ID), [ownUserRoles]);
};
...
export const useChannelCan = (channelId: number | undefined) => {
	const ownChannelPermissions = useChannelPermissionsById(channelId || -1);
	const isOwner = useIsOwnUserOwner();
	...
	const can = useCallback(
		(permission: ChannelPermission) => {
			...
			const permissions = ownChannelPermissions.permissions ?? {};
		...
		[ownChannelPermissions, isOwner, channel],
	);
...
export const useUserRoles = (userId: number) => {
	const roles = useServerStore((state) => state.roles);
	const user = useServerStore((state) => state.users.find((entry) => entry.id === userId));
...
export const useVoiceUsersByChannelId = (channelId: number) => {
	const users = useServerStore((state) => state.users);
	const voiceState = useServerStore((state) => voiceChannelStateSelector(state, channelId));
	...
	return useMemo(() => {
		...
		return Object.entries(voiceState.users).reduce<TVoiceUser[]>((voiceUsers, [userIdStr, userState]) => {
