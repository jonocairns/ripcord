...
export const useChannelsByCategoryId = (categoryId: number) => {
	...
	return useMemo(
		() => channels.filter((channel) => channel.categoryId === categoryId).sort((a, b) => a.position - b.position),
		[categoryId, channels],
	);
};
...
export const useLastTextChannelId = () => useServerStore(lastTextChannelIdSelector);

export const useCurrentVoiceChannelId = () => useServerStore(currentVoiceChannelIdSelector);

export const useIsCurrentVoiceChannelSelected = () => useServerStore(isCurrentVoiceChannelSelectedSelector);
...
export const useSelectedChannelType = () => useServerStore(selectedChannelTypeSelector);
