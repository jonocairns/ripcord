...
function parseTscOutput(output: string, repoRoot: string): TscError[] {
	// Standard tsc format: `path/to/file.ts(line,col): error TSxxxx: message`.
	// Bun's filtered runner prefixes lines with the workspace name; the path inside
	// is still a repo-rooted relative path that we resolve below.
	const re = /([^\s:|]+\.[mc]?tsx?)\((\d+),(\d+)\):\s*error\s+(TS\d+):\s*(.+)/g;
...
async function main() {
	...
	const allErrors = parseTscOutput(output, repoRoot);

	const inScope = changed.length > 0
		? allErrors.filter((e) => changed.some((c) => c === e.file || e.file.endsWith(c)))
		: allErrors;
